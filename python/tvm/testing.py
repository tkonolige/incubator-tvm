...
def requires_micro(*args):
    ...
    """
    _requires_micro = [
        pytest.mark.skipif(tvm.support.libinfo().get("USE_MICRO", "OFF") != "ON", reason="MicroTVM support not enabled. Set USE_MICRO=ON in config.cmake to enable.")
    ]
...
def requires_rpc(*args):
    ...
    """
    _requires_micro = [
        pytest.mark.skipif(tvm.support.libinfo().get("USE_RPC", "OFF") != "ON", reason="RPC support not enabled. Set USE_RPC=ON in config.cmake to enable.")
    ]
    return _compose(args, _requires_micro)
