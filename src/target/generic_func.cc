...
void GenericFunc::RegisterGenericFunc(GenericFunc func, const std::string& name) {
  ...
  std::lock_guard<std::mutex>(m->mutex);
  auto it = m->fmap.find(name);
  TVM_ICHECK(it == m->fmap.end()) << "GenericFunc already registered " << name;
  func->name_ = name;
  m->fmap[name] = func;
}
...
GenericFunc& GenericFunc::set_default(const PackedFunc value, bool allow_override) {
  ...
  if (!allow_override) {
    TVM_ICHECK(node->generic_func_ == nullptr)
...
GenericFunc& GenericFunc::register_func(const std::vector<std::string>& tags,
  ...
  for (auto& t : tags) {
    if (!allow_override) {
      ...
      TVM_ICHECK(iter == (*this)->dispatch_dict_.end())
...
void GenericFunc::CallPacked(TVMArgs args, TVMRetValue* ret) const {
  ...
  if (func == nullptr) {
    TVM_ICHECK(node->generic_func_ != nullptr) << "No generic function registered for " << node->name_;
