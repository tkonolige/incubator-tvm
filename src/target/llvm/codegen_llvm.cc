...
void CodeGenLLVM::InitTarget(llvm::TargetMachine* tm) {
  ...
  if (native_vector_bits_ == 0) {
    ...
    if (arch == llvm::Triple::x86_64) {
      ...
      TVM_LOG(WARNING) << "Set native vector bits to be 128 for " << arch_name;
    }
  }
}
...
void CodeGenLLVM::AddFunctionInternal(const PrimFunc& f, bool ret_void) {
  ...
  TVM_ICHECK_EQ(f->buffer_map.size(), 0U)
  ...
  TVM_ICHECK(module_->getFunction(static_cast<std::string>(global_symbol.value())) == nullptr)
...
std::unique_ptr<llvm::Module> CodeGenLLVM::Finish() {
  ...
  for (size_t i = 0; i < link_modules_.size(); ++i) {
    TVM_ICHECK(!llvm::Linker::linkModules(*module_, std::move(link_modules_[i])))
...
void CodeGenLLVM::HandleImport(const std::string& code) {
  ...
  if (code.length() >= 3 &&
    ...
    if (mlib.get() == nullptr) {
      ...
      TVM_LOG(FATAL) << "Fail to load bitcode file " << code << "\n"
    ...
    if (mlib.get() == nullptr) {
      std::string msg = std::string(err.getMessage());
      TVM_LOG(FATAL) << "Fail to load llvm ir "
                 << "line " << err.getLineNo() << ":" << msg << "\ncontent:\n"
                 << code;
    }
  }
  ...
  if (dtype.is_int() || dtype.is_uint()) {
    ...
    switch (dtype.bits()) {
      ...
      case 64:
        ...
        break;
      default:
        TVM_LOG(FATAL) << "do not support " << dtype;
    }
  }
  if (dtype.lanes() != 1) {
...
llvm::Type* CodeGenLLVM::GetLLVMType(const Type& type) const {
  if (auto* ptr = type.as<PrimTypeNode>()) {
    ...
    TVM_LOG(FATAL) << "Type " << type << " does not have a corresponding LLVM Type";
    return t_void_;
  }
}
...
llvm::Value* CodeGenLLVM::CreateVecSlice(llvm::Value* vec, int begin, int extent) {
  ...
  TVM_ICHECK(begin >= 0 && extent <= num_elems) << "Slicing out of bound!\n";
...
llvm::Value* CodeGenLLVM::CreateVecPad(llvm::Value* vec, int target_lanes) {
  ...
  TVM_ICHECK_LT(num_elems, target_lanes);
  for (int i = 0; i < num_elems; ++i) {
...
void CodeGenLLVM::CreateSerialFor(llvm::Value* begin, llvm::Value* end, llvm::Value* stride,
  ...
  TVM_ICHECK(!var_map_.count(loop_var.get()));
...
llvm::Value* CodeGenLLVM::CreateCast(DataType from, DataType to, llvm::Value* value) {
  ...
  if (to.is_handle()) {
    ...
    TVM_ICHECK(from.is_float() && to.is_float());
...
llvm::Value* CodeGenLLVM::GetVarValue(const VarNode* v) const {
  ...
  TVM_ICHECK(it != var_map_.end()) << "cannot find variable " << v->name_hint;
  return it->second;
}
...
llvm::Value* CodeGenLLVM::CreateIntrinsic(const CallNode* op) {
  if (op->op.same_as(builtin_call_llvm_intrin_) || op->op.same_as(builtin_call_llvm_pure_intrin_)) {
    ...
    TVM_ICHECK(f) << "Cannot find intrinsic declaration, possible type mismatch: "
    ...
    TVM_ICHECK_EQ(op->args[0].dtype().lanes(), 1) << "if_then_else can only take scalar condition";
    using llvm::BasicBlock;
    ...
    TVM_LOG(FATAL) << "CPU backend does not support atomic add yet.";
    return nullptr;
  } else {
    TVM_LOG(FATAL) << "unknown intrinsic " << op->op;
    return nullptr;
  }
}
  ...
  if (t.lanes() == 1) {
    ...
    if (const RampNode* ramp = op->index.as<RampNode>()) {
      if (is_one(ramp->stride)) {
        ...
        TVM_ICHECK_EQ(ramp->lanes, t.lanes());
...
llvm::Value* CodeGenLLVM::VisitExpr_(const ShuffleNode* op) {
  ...
  for (int i = 0, e = op->indices.size(); i < e; ++i) {
    ...
    TVM_ICHECK(val && *val >= 0 && *val < total_lanes) << "Shuffled indeces are suppose to be int, "
...
void CodeGenLLVM::VisitStmt_(const StoreNode* op) {
  TVM_ICHECK(is_one(op->predicate)) << op->predicate;
...
void CodeGenLLVM::VisitStmt_(const ForNode* op) {
  ...
  if (op->for_type == ForType::Unrolled) {
    TVM_LOG(WARNING) << "Unroll hint get ignore at CodeGenLLVM backend, "
                 << " consider set unroll_explicit=True";
  } else {
    TVM_ICHECK(op->for_type == ForType::Serial);
  }
...
void CodeGenLLVM::VisitStmt_(const AllocateNode* op) {
  ...
  TVM_ICHECK_GT(constant_size, 0) << "Can only handle constant size stack allocation";
