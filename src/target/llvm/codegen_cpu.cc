...
void CodeGenCPU::AddDebugInformation(llvm::Function* function) {
  ...
  TVM_ICHECK(!function->getSubprogram());
  ...
  TVM_ICHECK(DIFunction);
  function->setSubprogram(DIFunction);
  TVM_ICHECK_EQ(function->getSubprogram(), DIFunction);
...
llvm::DIType* CodeGenCPU::getDebugType(IRBuilder* builder, llvm::DIBuilder* di_builder,
  ...
  if (ty == builder->getVoidTy()) {
    ...
    ty->print(rso);
    TVM_LOG(FATAL) << "Unknown LLVM type:" << rso.str();
  }
  return nullptr;
}
...
void CodeGenCPU::AddMainFunction(const std::string& entry_func_name) {
  ...
  TVM_ICHECK(f) << "Function " << entry_func_name << "does not in module";
...
llvm::Value* CodeGenCPU::CreateStructRefPtr(DataType t, llvm::Value* buf, llvm::Value* index,
  ...
  if (kind < builtin::kArrKindBound_) {
    if (buf->getType() == t_void_p_) {
      ...
      TVM_ICHECK_EQ(buf->getType(), t_tvm_array_->getPointerTo());
    }
  }
  switch (kind) {
    ...
    case builtin::kTVMValueContent: {
      TVM_ICHECK_EQ(t.lanes(), 1);
      TVM_ICHECK(t.is_handle() || t.bits() == 64);
      if (t.is_int()) {
...
void CodeGenCPU::CreateParallelLaunch(const Stmt& body, int num_task) {
  ...
  std::swap(parallel_env_, par_env);
  std::swap(function_, f);
  TVM_ICHECK_NE(par_env.parallel_loop_count, 0) << "Cannot find parallel loop within parallel launch";
...
void CodeGenCPU::CreateStaticInit(const std::string& init_fname, const Stmt& body) {
  ...
  std::unordered_map<const VarNode*, llvm::Value*> new_vmap;
  UnpackClosureData(cdata, vfields, &new_vmap);
  TVM_ICHECK(parallel_env_.penv == nullptr);
  std::swap(function_, f);
  std::swap(var_map_, new_vmap);
  this->VisitStmt(body);
  builder_->CreateRet(ConstInt32(0));
...
void CodeGenCPU::AddStartupFunction() {
  if (registry_functions_.size() != 0) {
    TVM_ICHECK(is_system_lib_) << "Loading of --system-lib modules is yet to be defined for C runtime";
...
llvm::Value* CodeGenCPU::CreateIntrinsic(const CallNode* op) {
  if (op->op.same_as(builtin::tvm_call_packed_lowered())) {
    ...
    TVM_ICHECK(kind != builtin::kArrAddr);
    ...
    return WithFunctionEntry([&]() -> llvm::AllocaInst* {
      ...
      TVM_ICHECK(pval) << "require stack alloca to contain constant value";
      ...
      if (type == "shape") {
        ...
        TVM_LOG(FATAL) << "Unknown stack alloca type " << type;
        return nullptr;
      }
    });
  } else {
...
void CodeGenCPU::VisitStmt_(const AttrStmtNode* op) {
  if (op->attr_key == tir::attr::coproc_uop_scope) {
    ...
    if (op->attr_key == "pragma_parallel_stride_pattern") {
      ...
      TVM_ICHECK(parallel_env_.penv != nullptr) << "Cannot run barrier without parallel environment";
      TVM_ICHECK(!parallel_env_.in_parallel_loop)
      ...
      TVM_LOG(WARNING) << "Unknown pragma " << op->attr_key;
      this->VisitStmt(op->body);
    }
  } else {
    CodeGenLLVM::VisitStmt_(op);
...
void CodeGenCPU::VisitStmt_(const ForNode* op) {
  ...
  if (op->for_type == ForType::Serial || op->for_type == ForType::Unrolled) {
    ...
    if (parallel_env_.penv == nullptr) {
      ...
      TVM_ICHECK(parallel_env_.task_id.defined());
      TVM_ICHECK(parallel_env_.num_task.defined());
      TVM_ICHECK(parallel_env_.penv != nullptr);
    ...
    TVM_LOG(FATAL) << "cannot handle for type " << op->for_type;
  }
}
