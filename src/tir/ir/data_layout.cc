...
const LayoutAxis& LayoutAxis::Get(const char name) {
  TVM_ICHECK((name >= 'A' && name <= 'Z') || (name >= 'a' && name <= 'z'))
...
const LayoutAxis& LayoutAxis::Get(const IterVar& itvar) {
  ...
  TVM_ICHECK_EQ(axis.size(), 1) << "Invalid layout axis " << axis;
  return LayoutAxis::Get(axis[0]);
}

const LayoutAxis& LayoutAxis::Get(const std::string& name) {
  TVM_ICHECK_EQ(name.length(), 1) << "Invalid axis " << name;
...
Layout::Layout(const Array<IterVar>& axes) {
  ...
  for (const IterVar& axis : axes) {
    if (const auto* factor = axis->dom->extent.as<IntImmNode>()) {
      TVM_ICHECK_GT(factor->value, 0);
      repr << factor->value;
    }
    TVM_ICHECK_EQ(axis->var.get()->name_hint.size(), 1)
        << "Invalid layout axis " << axis->var.get()->name_hint;
    char c = axis->var.get()->name_hint.operator std::string()[0];
    TVM_ICHECK((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) << "Invalid layout axis " << c;
...
Layout::Layout(const std::string& name) {  // NOLINT(*)
  ...
  for (char c : name) {
    if (c >= 'A' && c <= 'Z') {
      TVM_ICHECK_EQ(factor, 0) << "Invalid layout " << name << ": invalid factor size " << factor
                           << " before dimension " << c;
      ...
      TVM_ICHECK_GT(factor, 0) << "Invalid layout " << name << ": invalid factor size " << factor
                           << " for dimension " << c;
      IterVar axis =
      ...
      TVM_ICHECK(factor >= 0) << "Invalid layout " << name << ": _ is adjacent to a number.";
      factor = factor * 10 + c - '0';
    } else {
      TVM_LOG(FATAL) << "Invalid layout " << name;
    }
  }
  ...
  for (const IterVar& v : node->axes) {
    ...
    TVM_ICHECK_EQ(axis_str.size(), 1);
    char axis = axis_str[0];
    TVM_ICHECK((axis >= 'a' && axis <= 'z') || (axis >= 'A' && axis <= 'Z'));
    TVM_ICHECK(!exist_axis[axis]) << "Invalid layout " << name << ": duplicate axis " << axis;
    exist_axis[axis] = true;
  }
  for (const IterVar& v : node->axes) {
    char axis = v->var.get()->name_hint.operator std::string()[0];
    if (axis >= 'a' && axis <= 'z') {
      TVM_ICHECK(exist_axis[axis - 'a' + 'A'])
...
Layout Layout::Split(const LayoutAxis& axis, size_t target_pos, int32_t factor) const {
  ...
  const auto axes = operator->()->axes;
  TVM_ICHECK(target_pos <= this->ndim())
      << "Invalid split position " << target_pos << " for layout " << name;
  TVM_ICHECK(axis.IsPrimal()) << "Cannot split a subordinate axis " << axis;
  TVM_ICHECK(this->Contains(axis)) << "Axis " << axis << " does not exist in " << name;
  TVM_ICHECK(!this->Contains(axis.ToSubordinate()))
      << "Axis " << axis << " has already been split in " << name;
  TVM_ICHECK(factor > 0) << "Invalid split size " << factor;
  Array<IterVar> new_layout;
...
Array<PrimExpr> BijectiveLayout::ForwardIndex(const Array<PrimExpr>& src_index) const {
  TVM_ICHECK(defined()) << "Cannot operate on an undefined bijective layout.";
  const BijectiveLayoutNode* self = operator->();
  TVM_ICHECK_EQ(src_index.size(), self->src_layout->axes.size())
...
Array<PrimExpr> BijectiveLayout::BackwardIndex(const Array<PrimExpr>& dst_index) const {
  TVM_ICHECK(defined()) << "Cannot operate on an undefined bijective layout.";
  const BijectiveLayoutNode* self = operator->();
  TVM_ICHECK_EQ(dst_index.size(), self->dst_layout->axes.size())
...
inline Array<PrimExpr> TransformShape(const Array<PrimExpr>& src_shape,
                                      ...
                                      const Array<PrimExpr>& transform_rule) {
  arith::Analyzer ana;
  TVM_ICHECK_EQ(src_shape.size(), src_axis.size());
  ...
  for (size_t i = 0; i < src_shape.size(); ++i) {
    ...
    if (!LayoutAxis::Get(orig_axis).IsPrimal()) {
      if (orig_shape.defined()) {
        ...
        if (orig_shape_const) {
          TVM_ICHECK_EQ(orig_shape_const->value, orig_axis_extent->value)
  ...
  Array<PrimExpr> result;
  TVM_ICHECK_EQ(transform_rule.size(), target_axis.size());
...
BijectiveLayout::BijectiveLayout(Layout src_layout, Layout dst_layout) {
  ...
  if (GetStoreRule(&n->forward_rule, n->src_layout, n->dst_layout)) {
    TVM_ICHECK(GetStoreRule(&n->backward_rule, n->dst_layout, n->src_layout));
    data_ = std::move(n);
  }
}
