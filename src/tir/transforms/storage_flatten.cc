...
class StorageFlattener : public StmtExprMutator {
 public:
  ...
  Stmt VisitStmt_(const StoreNode* op) final {
    ...
    if (it != var_remap_.end() && !it->second.same_as(op->buffer_var)) {
      TVM_ICHECK(it->second.as<VarNode>());
  ...
  Stmt VisitStmt_(const AttrStmtNode* op) final {
    if (op->attr_key == attr::realize_scope) {
      ...
      TVM_ICHECK(it != buf_map_.end()) << "Cannot find allocated buffer for " << buffer;
      ...
      TVM_ICHECK(tuple && tuple->op.same_as(builtin::tvm_tuple()));
  ...
  Stmt VisitStmt_(const BufferStoreNode* op) final {
    ...
    TVM_ICHECK(it != buf_map_.end()) << "Cannot find allocated buffer for " << key;

    const BufferEntry& e = it->second;
    TVM_ICHECK(!e.released) << "Read a buffer that is already out of scope";
  ...
  Stmt VisitStmt_(const BufferRealizeNode* op) final {
    ...
    if (buf_map_.count(key)) {
      TVM_ICHECK(buf_map_.at(key).external);
      return this->VisitStmt(op->body);
    } else {
      ...
      TVM_ICHECK(it != storage_scope_.end()) << "Cannot find storage scope of " << op->buffer;
      StorageScope skey;
      ...
      if (skey.tag.length() != 0) {
        ...
        if (info.defined()) {
          ...
          TVM_ICHECK_LE(const_size * dtype.bits(), info->max_num_bits)
  ...
  Stmt VisitStmt_(const PrefetchNode* op) final {
    ...
    TVM_ICHECK(!e.released) << "Read a buffer that is already out of scope";
    TVM_ICHECK_EQ(e.buffer->shape.size(), op->bounds.size())
  ...
  PrimExpr VisitExpr_(const ProducerLoadNode* op) final {
    TVM_LOG(FATAL) << "ProducerLoad cannot appear in a valid TIR PrimFunc.";
    return PrimExpr();
  }

  Stmt VisitStmt_(const ProducerStoreNode* op) final {
    TVM_LOG(FATAL) << "Cannot handle Provide "
               << " please run SchedulePostProcToPrimFunc first";
    return Stmt();
  }

  Stmt VisitStmt_(const ProducerRealizeNode* op) final {
    TVM_LOG(FATAL) << "Cannot handle Realize "
               << " please run SchedulePostProcToPrimFunc first";
    return Stmt();
  }
 ...
 private:
  ...
  Stmt HandleBufferBindScope(const AttrStmtNode* op) {
    ...
    TVM_ICHECK_EQ(arr.size(), 2U);
    const BufferNode* buffer = arr[0].as<BufferNode>();
    const BufferNode* target = arr[1].as<BufferNode>();
    const CallNode* tuple = op->value.as<CallNode>();
    TVM_ICHECK(buffer && target);
    TVM_ICHECK(tuple && tuple->op.same_as(builtin::tvm_tuple()));
    auto key = GetRef<Buffer>(target);

    auto it = buf_map_.find(key);
    TVM_ICHECK(it != buf_map_.end()) << "Cannot find buffer of " << key;
    const BufferEntry& be = it->second;
    TVM_ICHECK(!be.released);
    TVM_ICHECK_EQ(tuple->args.size(), be.buffer->shape.size() * 2);
    Array<PrimExpr> begins, extents;
    if (be.bounds.size() != 0) {
      TVM_ICHECK_EQ(tuple->args.size(), be.bounds.size() * 2);
    ...
    if (buffer->strides.size() == 0) {
      TVM_ICHECK_EQ(slice->strides.size(), 0U)
  ...
  struct BufferEntry {
    ...
    inline Array<PrimExpr> RelIndex(Array<PrimExpr> args) const {
      if (bounds.size() != 0) {
        ...
        TVM_ICHECK_EQ(bounds.size(), args.size());
