...
class LinearAccessPatternFinder final : public StmtExprVisitor {
 public:
  ...
  void VisitStmt_(const AllocateNode* op) final {
    ...
    TVM_ICHECK(it != alloc_info_.end()) << "Could not find buffer `" << buf->name_hint
                                    << "` in the list of allocated buffers. Perhaps you are "
                                       "missing a storage_scope attr for this buffer.";
    TVM_ICHECK(it->second.alloc == nullptr);
    it->second.alloc = op;
  ...
  void VisitStmt_(const StoreNode* op) final {
    ...
    if (it != alloc_info_.end() && it->second.alloc) {
      TVM_ICHECK_LT(it->second.level, scope_.size());
  ...
  void VisitExpr_(const LoadNode* op) final {
    ...
    auto it = alloc_info_.find(buf);
    if (it != alloc_info_.end() && it->second.alloc) {
      TVM_ICHECK_LT(it->second.level, scope_.size()) << "Load memory in places other than store.";
      scope_[it->second.level].touched.push_back(buf);
    }
  }
  ...
  void VisitExpr_(const VarNode* buf) final {
    ...
    auto it = alloc_info_.find(buf);
    if (it != alloc_info_.end() && it->second.alloc) {
      TVM_ICHECK_LT(it->second.level, scope_.size()) << " buf=" << buf->name_hint;
      scope_[it->second.level].touched.push_back(buf);
    }
  }
  template <typename T>
  void VisitNewScope(const T* op) {
    ...
    TVM_ICHECK_GT(end_index, begin_index);
    e.scope_pair_offset = begin_index - end_index;
    linear_seq_.push_back(e);
    // record the pointer to end index.
    TVM_ICHECK_NE(end_index, 0U);
...
class StoragePlanRewriter : public StmtExprMutator {
 public:
  ...
  PrimExpr VisitExpr_(const VarNode* op) final {
    ...
    if (it != alloc_map_.end()) {
      if (it->second->bits_offset != 0) {
        TVM_LOG(WARNING) << "Use a merged buffer variable address, could cause error";
      }
  ...
  PrimExpr VisitExpr_(const CallNode* op) final {
    if (op->op.same_as(builtin::tvm_access_ptr())) {
      ...
      TVM_ICHECK_EQ(se->bits_offset % elem_bits, 0U);
      if (se->bits_offset != 0) {
  ...
  Stmt VisitStmt_(const ForNode* op) final {
    TVM_ICHECK(op->for_type != ForType::Vectorized) << "VectorizeLoop before LiftStorageAlloc";
    // remake all the allocation at the attach scope.
    if (attach_map_.count(op)) {
      auto& svec = attach_map_[op];
      Stmt stmt = StmtExprMutator::VisitStmt_(op);
 ...
 private:
  ...
  PrimExpr RemapIndex(DataType dtype, PrimExpr index, StorageEntry* e) {
    ...
    uint64_t elem_bits = dtype.bits() * dtype.lanes();
    TVM_ICHECK_EQ(e->bits_offset % elem_bits, 0U);
    ...
    for (auto& kv : attach_map_) {
      ...
      for (size_t i = 0; i < vec.size(); ++i) {
        ...
        if (e->scope.tag.length() != 0) {
          TVM_ICHECK_NE(e->const_nbits, 0U) << "Special tagged memory must be const size";
      ...
      for (size_t i = 0; i < vec.size(); ++i) {
        ...
        if (e->allocs.size() == 1) {
          ...
          if (e->scope.tag.length() != 0) {
            ...
            TVM_ICHECK_LE(total_elem * e->elem_type.bits(), info->max_num_bits)
          ...
          for (const AllocateNode* op : e->allocs) {
            ...
            if (const auto* imm = sz.as<IntImmNode>()) {
              if (imm->value > std::numeric_limits<int>::max() / nbits) {
                TVM_LOG(WARNING) << "The allocation requires : " << imm->value << " * " << nbits
  ...
  void NewAllocTagMerged(StorageEntry* e) {
    TVM_ICHECK_NE(e->scope.tag.length(), 0U);
    // allocate with element type.
    TVM_ICHECK_NE(e->const_nbits, 0U);
    MemoryInfo info = GetMemoryInfo(e->scope.to_string());
    ...
    for (StorageEntry* child : e->merged_children) {
      TVM_ICHECK_NE(child->const_nbits, 0U);
      TVM_ICHECK_NE(total_bits, 0U);
    ...
    if (info.defined()) {
      TVM_ICHECK_LE(total_bits, info->max_num_bits)
          << "Allocation exceed bound of memory tag " << e->scope.to_string();
    }
  }
    ...
    for (size_t i = 0; i < seq.size(); ++i) {
      ...
      if (it != event_map_.end() && seq[i].scope_pair_offset >= 0) {
        ...
        for (const VarNode* var : it->second.gen) {
          TVM_ICHECK(alloc_info.count(var));
      ...
      if (s.stmt->IsInstance<AttrStmtNode>()) {
        ...
        if (op->attr_key == attr::thread_extent || op->attr_key == attr::virtual_thread ||
          ...
          TVM_ICHECK(op->attr_key == attr::extern_scope);
        }
  ...
  void Free(const VarNode* var) {
    ...
    TVM_ICHECK(it != alloc_map_.end());
    StorageEntry* e = it->second;
    TVM_ICHECK_NE(e->allocs.size(), 0U);
...
PrimFunc PointerValueTypeRewrite(PrimFunc f) {
  ...
  TVM_ICHECK_EQ(args.size(), n->params.size());
  n->params = args;
