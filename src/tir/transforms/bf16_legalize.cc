  ...
  std::tuple<PrimExpr, PrimExpr> DoCast(PrimExpr orig_a, PrimExpr orig_b, bool* is_bfloat16) {
    ...
    *is_bfloat16 = false;
    if (a->dtype.is_bfloat16()) {
      TVM_ICHECK(b->dtype.is_bfloat16());
      *is_bfloat16 = true;
    } else if (b->dtype.is_bfloat16()) {
      TVM_ICHECK(a->dtype.is_bfloat16());
      *is_bfloat16 = true;
    }
 ...
 public:
  ...
  PrimExpr VisitExpr_(const CastNode* op) final {
    ...
    if (op->value->dtype.is_bfloat16()) {
      ...
      TVM_ICHECK(op->dtype.is_float() && op->dtype.bits() == 32);
      ...
      TVM_ICHECK(op->value->dtype.is_float() && op->value->dtype.bits() == 32);
      auto uint32_dtype = DataType(kDLUInt, 32, op_val->dtype.lanes());
  ...
  PrimExpr VisitExpr_(const LoadNode* op) final {
    ...
    if (op->dtype.is_bfloat16()) {
      auto it = var_remap_.find(op->buffer_var);
      TVM_ICHECK(it != var_remap_.end()) << "bfloat* var needs to be remapped";
