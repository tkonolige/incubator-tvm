...
class VTInjector : public StmtExprMutator {
 public:
  ...
  PrimExpr VisitExpr_(const VarNode* op) final {
    TVM_ICHECK(!alloc_remap_.count(op)) << "Buffer address may get rewritten in virtual thread";
  ...
  Stmt VisitStmt_(const IfThenElseNode* op) final {
    ...
    visit_touched_var_ = false;
    TVM_ICHECK_EQ(max_loop_depth_, 0);
...
class VirtualThreadInjector : public StmtMutator {
 public:
  ...
  Stmt VisitStmt_(const ProducerStoreNode* op) final {
    TVM_LOG(FATAL) << "Need to call StorageFlatten first";
    return GetRef<Stmt>(op);
  }
};
