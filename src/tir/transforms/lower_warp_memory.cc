  ...
  void VisitStmt_(const StoreNode* op) final {
    if (op->buffer_var.get() == buffer_) {
      if (op->value.dtype().lanes() == 1) {
        ...
        TVM_ICHECK(arith::ramp(base, 1, op->value.dtype().lanes()).Match(op->index))
...
class WarpIndexFinder : private StmtVisitor {
 public:
  ...
  std::pair<Var, int> Find(const Stmt& stmt) {
    this->VisitStmt(stmt);
    TVM_ICHECK(warp_index_.defined())
  ...
  void VisitStmt_(const AttrStmtNode* op) final {
    if (op->attr_key == attr::thread_extent) {
      ...
      if (iv->thread_tag == "threadIdx.x") {
        ...
        TVM_ICHECK(value_as_int && value_as_int->value <= warp_size_ &&
        ...
        if (warp_index_.defined()) {
          TVM_ICHECK(warp_index_.same_as(iv))
...
class WarpAccessRewriter : protected StmtExprMutator {
 public:
  ...
  Stmt Rewrite(const AllocateNode* op) {
    ...
    TVM_ICHECK_GT(alloc_size, 0) << "warp memory only support constant alloc size";
 ...
 protected:
  PrimExpr VisitExpr_(const VarNode* op) override {
    TVM_ICHECK(op != buffer_) << "Cannot access address of warp memory directly";
  ...
  PrimExpr VisitExpr_(const LoadNode* op) override {
    if (op->buffer_var.get() == buffer_) {
      ...
      TVM_ICHECK(!ExprUseVar(local_index, warp_index_))
  ...
  std::pair<PrimExpr, PrimExpr> SplitIndexByGroup(const PrimExpr& index) {
    if (index.dtype().lanes() != 1) {
      ...
      arith::PVar<PrimExpr> base;
      TVM_ICHECK(arith::ramp(base, 1, index.dtype().lanes()).Match(index));
