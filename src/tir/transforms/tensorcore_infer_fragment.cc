  ...
  void VisitExpr_(const CallNode* op) final {
    ...
    if (op->op.same_as(builtin::tvm_load_matrix_sync()) ||
      ...
      if (fragments.count(buffer_var)) {
        ...
        TVM_ICHECK_EQ(m->value, info.m);
        TVM_ICHECK_EQ(n->value, info.n);
        TVM_ICHECK_EQ(k->value, info.k);
        if (scope == "wmma.matrix_a" || scope == "wmma.matrix_b") {
          TVM_ICHECK_EQ(layout->value, info.layout);
        }
      } else {
        // store metadata
        FragmentInfo info;
      ...
      TVM_ICHECK_EQ(scope, "wmma.accumulator");
      if (fragments.count(buffer_var)) {
        FragmentInfo info = fragments[buffer_var];
        TVM_ICHECK_EQ(m->value, info.m);
        TVM_ICHECK_EQ(n->value, info.n);
...
class FragmentChecker : public StmtExprVisitor {
 public:
  ...
  void VisitExpr_(const CallNode* op) final {
    ...
    if (op->op.same_as(builtin::tvm_mma_sync()) || op->op.same_as(builtin::tvm_bmma_sync())) {
      ...
      TVM_ICHECK(buffer_var_d);
      TVM_ICHECK(buffer_var_a);
      TVM_ICHECK(buffer_var_b);
      TVM_ICHECK(buffer_var_c);

      // Check all fragment A, B, C and D have the same shape
      TVM_ICHECK(CheckShape(buffer_var_d, buffer_var_a));
      TVM_ICHECK(CheckShape(buffer_var_d, buffer_var_b));
      TVM_ICHECK(CheckShape(buffer_var_d, buffer_var_c));
    }
  }
 ...
 private:
  ...
  bool CheckShape(const VarNode* buffer1, const VarNode* buffer2) {
    TVM_ICHECK(fragment_getter.fragments.count(buffer1));
    TVM_ICHECK(fragment_getter.fragments.count(buffer2));
