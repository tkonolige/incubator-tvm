...
class ThreadAllreduceBuilder final : public StmtExprMutator {
 ...
 private:
  ...
  Stmt MakeAllreduce(const CallNode* call) {
    TVM_ICHECK(!reduce_combiner_.empty());
    ...
    TVM_ICHECK(size_of_args) << call->args[0]->GetTypeKey();
    TVM_ICHECK_EQ(size, size_of_args->value);
    ...
    for (size_t i = 2 + 2 * size; i < call->args.size(); ++i) {
      ...
      if (v) {
        ...
        TVM_ICHECK(call->args[i].as<IntImmNode>() && call->args[i].as<IntImmNode>()->value == 0)
    ...
    for (const AttrStmtNode* attr : thread_extents_) {
      ...
      e.iv = iv;
      TVM_ICHECK_LE(e.scope.rank, 1);
      TVM_ICHECK_GE(e.scope.dim_index, 0) << "vthread do not work with cross thread reduction";
      if (e.scope.rank == 1) {
        const auto* ptr = attr->value.as<IntImmNode>();
        TVM_ICHECK(ptr) << "Need constant extent for reduce set " << iv;
    ...
    TVM_ICHECK_EQ(nmatch, reduce_set.size()) << "Not all reduce index are presented in the context";
    ...
    if (is_warp_reduction(types)) {
      ...
      TVM_ICHECK_EQ(reduce_extent, warp_size_) << "not a warp reduction";
      //
      ...
      for (size_t i = 0; i < size; ++i) {
        TVM_ICHECK(!load_remap_.count(buffers[i]));
        PrimExpr pred = const_true(types[i].lanes());
        Var var = shared_bufs[i];
      ...
      for (size_t idx = 0; idx < size; ++idx) {
        TVM_ICHECK(!load_remap_.count(buffers[idx]));
        PrimExpr pred = const_true(types[idx].lanes());
  ...
  Stmt MakeBufAllreduce(const CommReducerNode* combiner, const std::vector<DataType>& types,
    ...
    TVM_ICHECK(threadx_extent >= 1 && warp_size_ >= 1);
...
Pass LowerThreadAllreduce() {
  auto pass_func = [](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    TVM_ICHECK(target.defined()) << "LowerThreadAllreduce: Require the target attribute";
