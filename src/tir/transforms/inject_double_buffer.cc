...
class DoubleBufferInjector : public StmtExprMutator {
 public:
  ...
  Stmt VisitStmt_(const AllocateNode* op) final {
    ...
    if (it != dbuffer_info_.end()) {
      ...
      TVM_ICHECK(it->second.loop != nullptr);
  ...
  Stmt VisitStmt_(const ForNode* op) final {
    ...
    if (it != loop_pre_.end()) {
      ...
      if (split_loop_ != 0) {
        ...
        TVM_ICHECK(split_loop_ % 2 == 0 || split_loop_ == 1)
            << "It is better to split with multiple of 2";
        TVM_ICHECK(is_zero(old_loop->min));
  ...
  Stmt VisitStmt_(const StoreNode* op) final {
    ...
    if (it != dbuffer_info_.end()) {
      ...
      TVM_ICHECK(in_double_buffer_scope_);
      TVM_ICHECK(e.stride.defined());
  ...
  PrimExpr VisitExpr_(const LoadNode* op) final {
    ...
    if (it != dbuffer_info_.end()) {
      const StorageEntry& e = it->second;
      TVM_ICHECK(e.stride.defined());
      TVM_ICHECK(e.switch_read_var.defined());
  ...
  PrimExpr VisitExpr_(const VarNode* op) final {
    TVM_ICHECK(!dbuffer_info_.count(op));
 ...
 private:
  Stmt MakeProducer(const AttrStmtNode* op) {
    ...
    TVM_ICHECK_NE(loop_nest_.size(), 0U) << "Double buffer scope must be inside a loop";
    auto it = dbuffer_info_.find(buffer.get());
    if (it == dbuffer_info_.end()) {
      TVM_LOG(WARNING) << "Skip double buffer scope " << op->node;
      return this->VisitStmt(op->body);
    }
