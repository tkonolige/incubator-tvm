...
size_t InferTensorizeRegion(const ComputeOpNode* self, const Stage& stage,
  ...
  for (size_t i = stage->leaf_iter_vars.size(); i != 0; --i) {
    ...
    TVM_ICHECK(iv->iter_type == kDataPar || iv->iter_type == kCommReduce);
    auto vit = dom_map.find(iv);
    TVM_ICHECK(vit != dom_map.end());
    const Range& vrange = vit->second;
    if (is_one(vrange->extent)) {
      up_state[iv] = IntSet::SinglePoint(vrange->min);
    } else if (found_point) {
      TVM_ICHECK(is_zero(vrange->min));
    ...
    if (iit != stage->iter_var_attrs.end()) {
      ...
      if (!found_point) {
        TVM_ICHECK(!attr->bind_thread.defined()) << "Do not allow thread in tensorize scope";
      }
      if (attr->iter_type == kTensorized) {
        TVM_ICHECK(!found_point) << "Do not allow two tensorized point";
        found_point = true;
        loc_scope = i - 1;
      }
    }
  }
  TVM_ICHECK(found_point);
  ...
  for (const auto& kv : in_dom) {
    ...
    for (size_t i = 0; i < t.ndim(); ++i) {
      ...
      TVM_ICHECK(r.defined()) << "cannot deduce region of tensorized scope for input " << t;
...
void VerifyTensorizeLoopNest(const ComputeOpNode* self, const Stage& stage,
  ...
  TVM_ICHECK_EQ(n.main_nest.size(), stage->leaf_iter_vars.size() + 1);
  TVM_ICHECK(n.init_nest.size() == stage->leaf_iter_vars.size() + 1 || n.init_nest.size() == 0);
  ...
  for (const PrimExpr& pred : n.main_predicates) {
    if (tir::ExprUseVar(pred, fbanned)) {
      TVM_LOG(FATAL) << "Tensorize failed, split condition " << pred
 ...
 public:
  PrimExpr VisitExpr_(const ProducerLoadNode* op) final {
    ...
    if (it != in_remap_.end()) {
      ...
      TVM_ICHECK_EQ(op->indices.size(), e.region.size());
  ...
  void Init(const ComputeOpNode* self, const Stage& stage,
    ...
    // input remap.
    Array<Tensor> inputs = self->InputTensors();
    TVM_ICHECK_EQ(inputs.size(), intrin->inputs.size());
    for (size_t i = 0; i < inputs.size(); ++i) {
      InputEntry e;
      e.tensor = intrin->inputs[i];
      e.region = Array<Range>(in_region.at(inputs[i]));
      TVM_ICHECK_GE(e.region.size(), e.tensor.ndim());
      ...
      for (size_t j = 0; j < e.start; ++j) {
        ...
        TVM_ICHECK(is_one(canonical_extent))
    ...
    TVM_ICHECK(intrin_compute) << "Only support compute intrinsic for now";
    TVM_ICHECK_GE(self->axis.size(), intrin_compute->axis.size())
    ...
    for (size_t i = 0; i < axis_start; ++i) {
      ...
      TVM_ICHECK(is_one(r->extent)) << "Tensorize: Output mismatch with tensor intrin "
    ...
    TVM_ICHECK_GE(self->reduce_axis.size(), intrin_compute->reduce_axis.size())
    ...
    for (size_t i = 0; i < axis_start; ++i) {
      ...
      TVM_ICHECK(is_one(r->extent)) << "Tensorize: Reduction mismatch with tensor intrin "
...
void VerifyTensorizeBody(const ComputeOpNode* self, const Stage& stage,
  ...
  const ComputeOpNode* intrin_compute = intrin->op.as<ComputeOpNode>();
  TVM_ICHECK(intrin_compute) << "Only support compute intrinsic for now";
  TVM_ICHECK_EQ(body.size(), intrin_compute->body.size()) << "Tensorize failed: body size mismatch";
  arith::Analyzer ana;
  ...
  for (size_t i = 0; i < body.size(); ++i) {
    ...
    if (lhs.dtype() != rhs.dtype()) {
      TVM_LOG(FATAL) << "Failed to match the data type with TensorIntrin " << intrin->name
                 << "'s declaration "
                 << " provided=" << lhs.dtype() << ", intrin=" << rhs.dtype();
    }
    TVM_ICHECK(expr_equal(lhs, rhs)) << "Failed to match the compute with TensorIntrin " << intrin->name
                                 << "'s declaration "
...
Stmt MakeTensorize(const ComputeOpNode* self, const Stage& stage,
  ...
  Array<Tensor> inputs = self->InputTensors();
  TVM_ICHECK_EQ(inputs.size(), intrin->inputs.size()) << "Tensorize failed: input size mismatch ";
  // input binding
  ...
  const ComputeOpNode* intrin_compute = intrin->op.as<ComputeOpNode>();
  TVM_ICHECK(intrin_compute) << "Only support compute intrinsic for now";
  TVM_ICHECK_EQ(intrin->inputs.size() + intrin_compute->body.size(), intrin->buffers.size());
  TVM_ICHECK_EQ(intrin_compute->body.size(), self->body.size());
  Array<PrimExpr> tuple;
  for (IterVar iv : self->axis) {
    auto it = out_dom.find(iv);
    TVM_ICHECK(it != out_dom.end());
  ...
  for (size_t i = 0; i < start; ++i) {
    ...
    auto it = out_dom.find(iv);
    TVM_ICHECK(it != out_dom.end());
    TVM_ICHECK(is_one(it->second->extent)) << "Tensorization fail: reduction axis size do not match";
  }
  ...
  if (tloc <= n.num_common_loop) {
    ...
    TVM_ICHECK_EQ(n.init_predicates.size(), 0U);
    TVM_ICHECK(intrin->body.defined()) << "Normal store op for intrin " << intrin << " is not defined";
    ...
    if (intrin->reduce_init.defined()) {
      ...
      TVM_ICHECK(intrin->body.defined()) << "Normal body op for intrin " << intrin << " is not defined";
...
TVM_REGISTER_GLOBAL("test.op.InferTensorizeRegion").set_body([](TVMArgs args, TVMRetValue* ret) {
  ...
  std::unordered_map<IterVar, Range> out_dom;
  std::unordered_map<Tensor, Array<Range> > in_region;
  TVM_ICHECK(stage->op.as<ComputeOpNode>());
