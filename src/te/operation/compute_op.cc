...
void BaseComputeOpNode::GatherBound(const Operation& self,
  ...
  for (size_t i = 0; i < this->axis.size(); ++i) {
    ...
    TVM_ICHECK(!out_dom_map->count(this->axis[i]));
    (*out_dom_map)[this->axis[i]] = r;
  }
  for (size_t i = 0; i < this->reduce_axis.size(); ++i) {
    TVM_ICHECK(!out_dom_map->count(this->reduce_axis[i]));
...
ComputeType DetectComputeType(const ComputeOpNode* self, const Stage& stage) {
  ...
  for (IterVar iv : stage->leaf_iter_vars) {
    ...
    if (iv->iter_type == kCommReduce) {
      ...
      TVM_ICHECK_EQ(thread_red, 0) << "Cross thread reduce cannot swap with normal data axis";
    }
  }
  if (tensorize != 0) {
    TVM_ICHECK(thread_red == 0) << "Cannot mix cross thread reduction with Tensorize";
...
ComputeLoopNest ComputeLoopNest::Create(const BaseComputeOpNode* self, const Stage& stage,
                                        const std::unordered_map<IterVar, Range>& dom_map,
                                        bool debug_keep_trivial_loop) {
  TVM_ICHECK_EQ(stage->op.operator->(), self);
  ComputeLoopNest ret;
  // make main loop nest
  ...
  if (self->reduce_axis.size() != 0) {
    ...
    TVM_ICHECK_EQ(ret.main_nest.size(), stage->leaf_iter_vars.size() + 1);
...
class ComputeVerifier final : protected tir::ExprVisitor {
 public:
  ...
  void Run() {
    for (const PrimExpr e : compute_->body) {
      ...
      TVM_ICHECK((reduce && reduce_) || (!reduce && !reduce_)) << "All ComputeOp should be consistent "
                                                           << "with being Reduce operation or not.";

      if (reduce && reduce_) {
        TVM_ICHECK(ReduceEqual(reduce, reduce_)) << "The Reduce inputs of ComputeOp should "
 ...
 protected:
  ...
  void VisitExpr_(const tir::ReduceNode* op) final {
    ...
    TVM_ICHECK(0 == level_) << "Reductions are only allowed at the top level of compute. "
...
Stmt TransformUpdate(const Stage& stage, const std::unordered_map<IterVar, Range>& dom_map,
  ...
  for (const PrimExpr& pred : n.main_predicates) {
    if (tir::ExprUseVar(pred, fbanned)) {
      TVM_LOG(FATAL) << "Tensorize update transform failed, the condition " << pred
