...
Array<PrimExpr> ScanOpNode::output_shape(size_t i) const {
  TVM_ICHECK_LT(i, state_placeholder.size());
...
ScanOp::ScanOp(std::string name, std::string tag, Map<String, ObjectRef> attrs, IterVar axis,
  ...
  TVM_ICHECK_EQ(init.size(), update.size());
  TVM_ICHECK_EQ(init.size(), state_placeholder.size());
  ...
  for (size_t i = 0; i < init.size(); ++i) {
    TVM_ICHECK_EQ(init[i]->dtype, state_placeholder[i]->dtype);
    TVM_ICHECK_EQ(init[i]->dtype, update[i]->dtype);
    TVM_ICHECK(prove_equal(init[i]->shape[0], axis->dom->min))
        << "init.shape[0] need to match scan_axis.dom.min";
    TVM_ICHECK(prove_equal(state_placeholder[i]->shape[0], axis->dom->min + axis->dom->extent))
        << "state_placeholder.shape[0] need to match"
        << " scan_axis.dom.min + scan_axis.dom.extent";
    TVM_ICHECK_EQ(state_placeholder[i].ndim(), init[i].ndim())
        << "The dimension of init need to match state_placeholder";
    TVM_ICHECK_EQ(update[i].ndim(), state_placeholder[i].ndim())
        << "The update.ndim need to be state_placeholder.ndim - 1";
    for (size_t k = 0; k < update[i].ndim(); ++k) {
      TVM_ICHECK(prove_equal(update[i]->shape[k], state_placeholder[i]->shape[k]));
      if (k != 0) {
        // setup spatial axis
    ...
    for (size_t k = 1; k < init[i].ndim(); ++k) {
      TVM_ICHECK(prove_equal(init[i]->shape[k], state_placeholder[i]->shape[k]));
    }
  }
...
Operation ScanOpNode::ReplaceInputs(const Operation& self,
  ...
  TVM_ICHECK_EQ(self.operator->(), this);
...
void ScanOpNode::GatherBound(const Operation& self,
  ...
  TVM_ICHECK_EQ(self.operator->(), this);
  TVM_ICHECK(!out_dom_map->count(this->scan_axis));
  ...
  for (size_t i = 0; i < output.size(); ++i) {
    ...
    for (size_t k = 1; k < this->update[i]->shape.size(); ++k, ++sp_idx) {
      IterVar sp_ax = this->spatial_axis_[sp_idx];
      TVM_ICHECK(!out_dom_map->count(sp_ax));
      TVM_ICHECK(fix_pt.count(sp_ax));
...
Stmt ScanOpNode::BuildRealize(const Stage& stage, const std::unordered_map<IterVar, Range>& dom_map,
  ...
  arith::Analyzer analyzer;
  TVM_ICHECK_EQ(stage->op.get(), this);
  ...
  for (size_t i = 0; i < update.size(); ++i) {
    ...
    TVM_ICHECK_EQ(static_cast<size_t>(t->value_index), i);
    Region bounds;
...
Stmt ScanOpNode::BuildProvide(const Stage& stage, const std::unordered_map<IterVar, Range>& dom_map,
  ...
  TVM_ICHECK_EQ(stage->op.operator->(), this);
  Stmt provide =
