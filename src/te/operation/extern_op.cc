...
ExternOp::ExternOp(std::string name, std::string tag, Map<String, ObjectRef> attrs,
  ...
  TVM_ICHECK_EQ(inputs.size(), input_placeholders.size());
  for (size_t i = 0; i < inputs.size(); ++i) {
    TVM_ICHECK_EQ(inputs[i]->dtype, input_placeholders[i]->dtype);
    TVM_ICHECK_EQ(inputs[i]->shape.size(), input_placeholders[i]->shape.size());
    for (size_t dim = 0; dim < inputs[i]->shape.size(); ++dim) {
      TVM_ICHECK(inputs[i]->shape[dim].same_as(input_placeholders[i]->shape[dim]));
    }
    TVM_ICHECK_EQ(input_placeholders[i]->strides.size(), 0U);
  }
