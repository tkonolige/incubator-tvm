...
Tensor Schedule::cache_read(const Tensor& tensor, const std::string& scope,
  ...
  for (Operation op : readers) {
    ...
    TVM_ICHECK(!repl_op.same_as(s->op)) << "Cannot find " << tensor << " in the inputs of " << s->op;
...
void PrepareAxisMapping(Stage orig_stage, OpType* op, std::unordered_set<IterVar>* p_red_axis,
  ...
  te::PassDownDomain(orig_stage, &dom_map, &analyzer, true);
  {
    ...
    for (IterVar iv : orig_stage->leaf_iter_vars) {
      ...
      TVM_ICHECK_EQ(iv->iter_type, kDataPar) << "Can only relayout with in data parallel dimensions";
  ...
  for (auto cbody : compute->body) {
    ...
    if (body->IsInstance<tir::ReduceNode>()) {
      ...
      if (first_reduce != nullptr) {
        TVM_ICHECK(ReduceEqual(reduce_body, first_reduce));
      ...
      TVM_ICHECK(first_reduce == nullptr) << "cannot mix reduce and other node in ONE compute bodys";
    }
...
Array<Tensor> CacheWriteWithReLayoutTensor(Schedule sch, const Array<Tensor>& tensor_array,
  ...
  TVM_ICHECK_EQ(tensor_op->num_outputs(), 1)
...
Array<Tensor> Schedule::cache_write(const Array<Tensor>& tensor_array, const std::string& scope) {
  (*this)->InvalidateCache();
  TVM_ICHECK(tensor_array.size() > 0) << "size of tensor_array must be greater than 0";
  Tensor tensor = tensor_array[0];
  Stage orig_stage = operator[](tensor->op);
  const ComputeOpNode* compute = tensor->op.as<ComputeOpNode>();
  TVM_ICHECK(static_cast<size_t>(compute->num_outputs()) == tensor_array.size())
  ...
  for (size_t i = 1; i < tensor_array.size(); i++) {
    ...
    TVM_ICHECK(orig_stage.same_as(tmp_stage)) << "Input tensor list must be generated by ONE computeOp";
  }
...
Tensor Schedule::cache_write(const Tensor& tensor, const std::string& scope) {
  ...
  if (tensor->op.as<ComputeOpNode>()) {
    ...
    TVM_LOG(FATAL) << "cache write only take ComputeOp or TensorComputeOp as writers";
    return Tensor();
  }
}
  ...
  for (size_t i = sch->stages.size(); i != 0; --i) {
    ...
    if (stage->attach_type == kInline) {
      ...
      PrimExpr body;
      {
        ...
        TVM_ICHECK(compute) << "can only inline compute op";
        for (auto iv : compute->axis) {
          args.push_back(iv->var);
        }
        TVM_ICHECK_EQ(compute->body.size(), 1U) << "can only inline compute op with 1 output";
        ...
        if (compute) {
          ...
          if (new_body[j][0]->IsInstance<tir::ReduceNode>()) {
            ...
            for (size_t k = 1; k < new_body[j].size(); ++k) {
              ...
              TVM_ICHECK(reduce_);
              TVM_ICHECK(ReduceEqual(reduce_, reduce)) << "The Reduce inputs of ComputeOp should "
            ...
            if (!new_value.same_as(new_body[j][0])) {
              ...
              TVM_ICHECK(r != nullptr);
              TVM_ICHECK_EQ(new_body[j].size(), r->source.size());
...
Array<Tensor> Schedule::rfactor(const Tensor& tensor, const IterVar& axis, int factor_axis) {
  ...
  for (IterVar iv : compute_op->axis) {
    TVM_ICHECK(!touch_map.count(iv)) << "Factor axis touches normal axis.";
    skip_bound_check.insert(iv);
  }
  // get analyzer.
  arith::Analyzer analyzer;
  ...
  TVM_ICHECK_LE(factor_axis_pos, compute_op->axis.size());
  ...
  n->name = compute_op->name + ".rf";
  {
    ...
    TVM_ICHECK(is_zero(iv_node->dom->min)) << "Can only factor reduction domain starting from 0";
  ...
  TVM_ICHECK(reduce) << "Can only rfactor non-inline reductions";
  ...
  for (IterVar iv : reduce_stage->leaf_iter_vars) {
    if (touch_map.count(iv) && !iv.same_as(axis)) {
      TVM_ICHECK_EQ(iv->iter_type, kCommReduce);
  ...
  TVM_ICHECK_LT(stage_pos, stages.size());
        ...
        if (!reduce->init.empty()) {
          ...
          for (const auto& init : reduce->init) {
            if (init->IsInstance<ProducerLoadNode>()) {
              TVM_ICHECK_EQ(compute_op->axis.size(), idx_size)
