...
PackedFunc GraphRuntimeFactory::GetFunction(
  ...
  if (name == module_name_) {
    ...
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
      TVM_ICHECK_GE(args.size(), 2);
      std::string module_name = args[0].operator String();
      TVM_ICHECK(module_name == module_name_) << "Currently we only support single model for now.";
      std::vector<TVMContext> contexts;
...
Module GraphRuntimeFactory::DebugRuntimeCreate(const std::vector<TVMContext>& ctxs) {
  ...
  TVM_ICHECK(pf != nullptr) << "Cannot find function tvm.graph_runtime_debug.create in registry. "
...
Module GraphRuntimeFactoryModuleLoadBinary(void* strm) {
  ...
  TVM_ICHECK(stream->Read(&graph_json));
  uint64_t sz;
  TVM_ICHECK(stream->Read(&sz));
  std::vector<std::string> names;
  TVM_ICHECK(stream->Read(&names));
  TVM_ICHECK(sz == names.size());
  for (size_t i = 0; i < sz; ++i) {
  ...
  TVM_ICHECK(stream->Read(&module_name));
