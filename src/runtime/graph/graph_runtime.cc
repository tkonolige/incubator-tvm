...
void GraphRuntime::SetInput(int index, DLTensor* data_in) {
  TVM_ICHECK_LT(static_cast<size_t>(index), input_nodes_.size());
...
void GraphRuntime::SetInputZeroCopy(int index, DLTensor* data_ref) {
  ...
  TVM_ICHECK_EQ(data_alignment_[eid], details::GetDataAlignment(*data_ref));
  TVM_ICHECK_EQ(reinterpret_cast<size_t>(data_ref->data) % kAllocAlignment, 0);
  TVM_ICHECK_EQ(old_t->ndim, static_cast<size_t>(data_ref->ndim));
  TVM_ICHECK_EQ(old_t->ctx.device_type, data_ref->ctx.device_type);
  TVM_ICHECK_EQ(old_t->ctx.device_id, data_ref->ctx.device_id);
  for (auto i = 0; i < data_ref->ndim; ++i) {
    TVM_ICHECK_EQ(old_t->shape[i], data_ref->shape[i]);
  }
...
NDArray GraphRuntime::GetOutput(int index) const {
  TVM_ICHECK_LT(static_cast<size_t>(index), outputs_.size());
...
void GraphRuntime::CopyOutputTo(int index, DLTensor* data_out) {
  ...
  TVM_ICHECK_EQ(data->ndim, data_out->ndim);
  for (int32_t j = 0; j < data->ndim; ++j) {
    TVM_ICHECK_EQ(data->shape[j], data_out->shape[j]);
  }
...
void GraphRuntime::LoadParams(dmlc::Stream* strm) {
  ...
  TVM_ICHECK(strm->Read(&header)) << "Invalid parameters file format";
  TVM_ICHECK(header == kTVMNDArrayListMagic) << "Invalid parameters file format";
  TVM_ICHECK(strm->Read(&reserved)) << "Invalid parameters file format";

  std::vector<std::string> names;
  TVM_ICHECK(strm->Read(&names)) << "Invalid parameters file format";
  uint64_t sz;
  strm->Read(&sz);
  size_t size = static_cast<size_t>(sz);
  TVM_ICHECK(size == names.size()) << "Invalid parameters file format";
  for (size_t i = 0; i < size; ++i) {
    ...
    TVM_ICHECK_LT(eid, data_entry_.size());
...
void GraphRuntime::ShareParams(const GraphRuntime& other, dmlc::Stream* strm) {
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    uint32_t eid = this->entry_id(input_nodes_[in_idx], 0);
    TVM_ICHECK_LT(eid, data_entry_.size());
    TVM_ICHECK_EQ(data_entry_[eid].use_count(), 1);
    data_entry_[eid] = other.GetInput(GetInputIndex(names[i]));
    TVM_ICHECK_GT(data_entry_[eid].use_count(), 1);
...
void GraphRuntime::SetupStorage() {
  ...
  for (size_t i = 0; i < attrs_.shape.size(); ++i) {
    ...
    TVM_ICHECK_GE(storage_id, 0) << "Do not support runtime shape op";
    DLDataType t = vtype[i];
    size_t bits = t.bits * t.lanes;
    TVM_ICHECK(bits % 8U == 0U || bits == 1U || bits == 4U);
    ...
    if (sid >= pool_entry.size()) {
      ...
      TVM_ICHECK(pool_entry[sid].device_type == -1 || pool_entry[sid].device_type == device_type)
  ...
  for (size_t i = 0; i < data_entry_.size(); ++i) {
    int storage_id = attrs_.storage_id[i];
    TVM_ICHECK_LT(static_cast<size_t>(storage_id), storage_pool_.size());
...
void GraphRuntime::SetupOpExecs() {
  ...
  for (uint32_t nid = 0; nid < this->GetNumOfNodes(); ++nid) {
    ...
    TVM_ICHECK(inode.op_type == "tvm_op") << "Can only take tvm_op as op";
...
std::pair<std::function<void()>, std::shared_ptr<GraphRuntime::OpArgs> > GraphRuntime::CreateTVMOp(
  ...
  TVM_ICHECK(pf != nullptr) << "no such function in module: " << param.func_name;
...
PackedFunc GraphRuntime::GetFunction(const std::string& name,
  ...
  if (name == "set_input") {
    ...
    return PackedFunc([sptr_to_self, this](TVMArgs args, TVMRetValue* rv) {
      ...
      TVM_ICHECK_EQ(module.operator->()->type_key(), "GraphRuntime");
...
TVM_REGISTER_GLOBAL("tvm.graph_runtime.create").set_body([](TVMArgs args, TVMRetValue* rv) {
  TVM_ICHECK_GE(args.num_args, 4) << "The expected number of arguments for graph_runtime.create is "
