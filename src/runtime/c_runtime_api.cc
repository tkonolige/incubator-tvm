...
std::string GetCustomTypeName(uint8_t type_code) {
  ...
  TVM_ICHECK(f) << "Function runtime._datatype_get_type_name not found";
...
uint8_t GetCustomTypeCode(const std::string& type_name) {
  ...
  TVM_ICHECK(f) << "Function runtime._datatype_get_type_code not found";
...
bool GetCustomTypeRegistered(uint8_t type_code) {
  ...
  TVM_ICHECK(f) << "Function runtime._datatype_get_type_registered not found";
  return (*f)(type_code).operator bool();
}

uint8_t ParseCustomDatatype(const std::string& s, const char** scan) {
  TVM_ICHECK(s.substr(0, 6) == "custom") << "Not a valid custom datatype string";

  auto tmp = s.c_str();

  TVM_ICHECK(s.c_str() == tmp);
  *scan = s.c_str() + 6;
  TVM_ICHECK(s.c_str() == tmp);
  if (**scan != '[') TVM_LOG(FATAL) << "expected opening brace after 'custom' type in" << s;
  TVM_ICHECK(s.c_str() == tmp);
  *scan += 1;
  TVM_ICHECK(s.c_str() == tmp);
  ...
  if (*(*scan + custom_name_len) != ']')
    TVM_LOG(FATAL) << "expected closing brace after 'custom' type in" << s;
  TVM_ICHECK(s.c_str() == tmp);
...
class DeviceAPIManager {
 ...
 private:
  ...
  DeviceAPI* GetAPI(const std::string name, bool allow_missing) {
    ...
    if (f == nullptr) {
      TVM_ICHECK(allow_missing) << "Device API " << name << " is not enabled.";
      return nullptr;
    }
    void* ptr = (*f)();
...
TVMStreamHandle DeviceAPI::CreateStream(TVMContext ctx) {
  TVM_LOG(FATAL) << "Device does not support stream api.";
  return nullptr;
}
 ...
 *
 *  Parse them header generated by by TVM_LOG(FATAL) and TVM_ICHECK
