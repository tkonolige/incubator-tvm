...
inline void* HexagonDeviceAPI::AllocDataSpace(TVMContext ctx, size_t nbytes, size_t alignment,
  ...
  TVM_ICHECK(hexagon::Device::ValidateDeviceId(ctx.device_id));
...
inline void HexagonDeviceAPI::CopyDataFromTo(const void* from, size_t from_offset, void* to,
  ...
  if (ctx_from.device_type == ctx_to.device_type) {
    if (ctx_from.device_type == kDLCPU) {
      ...
      TVM_ICHECK(hexagon::Device::ValidateDeviceId(ctx_from.device_id));
      TVM_ICHECK_EQ(ctx_from.device_id, ctx_to.device_id);
      TVM_ICHECK(Is32bit(dst) && Is32bit(src));
      hexagon::Device::Global()->CopyDeviceToDevice(dst, src, num_bytes);
    }
  } else {
    if (ctx_from.device_type == kDLCPU) {
      TVM_ICHECK_EQ(static_cast<int>(ctx_to.device_type), kDLHexagon);
      TVM_ICHECK(Is32bit(dst));
      TVM_ICHECK(hexagon::Device::ValidateDeviceId(ctx_to.device_id));
      hexagon::Device::Global()->CopyHostToDevice(dst, src, num_bytes);
    } else {
      TVM_ICHECK_EQ(static_cast<int>(ctx_from.device_type), kDLHexagon);
      TVM_ICHECK_EQ(ctx_to.device_type, kDLCPU);
      TVM_ICHECK(Is32bit(src));
      TVM_ICHECK(hexagon::Device::ValidateDeviceId(ctx_from.device_id));
