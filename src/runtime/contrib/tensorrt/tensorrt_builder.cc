...
void TensorRTBuilder::AddInput(int nid, uint32_t entry_id, const JSONGraphNode& node) {
  ...
  TVM_ICHECK_EQ(shapes.size(), dtypes.size());
  ...
  for (size_t i = 0; i < shapes.size(); ++i) {
    ...
    TVM_ICHECK(TypeMatch(dtypes[i], kDLFloat, 32)) << "Only FP32 inputs are supported.";
...
void TensorRTBuilder::AddOutput(const JSONGraphNodeEntry& node, uint32_t entry_id) {
  ...
  TVM_ICHECK(it != node_output_map_.end()) << "Output was not found.";
...
void TensorRTBuilder::AddLayer(int nid, const JSONGraphNode& node) {
  ...
  TVM_ICHECK(it != GetOpConverters()->end())
  ...
  for (size_t i = 0; i < node.GetInputs().size(); ++i) {
    ...
    TVM_ICHECK(it != node_output_map_.end()) << "Input was not found.";
    ...
    if (!converter->variable_input_count) {
      if (converter->input_types[i] == kTensor && input.type == kWeight) {
        ...
        TVM_LOG(FATAL) << "Input " << i << " for " << params.op_name
                   << " requires weights but got a tensor.";
      }
    }
    params.inputs.push_back(input);
  }
  TVM_ICHECK(converter->variable_input_count || converter->input_types.size() == params.inputs.size())
...
#endif
  TVM_ICHECK_EQ(engine->getNbBindings(), network_input_names_.size() + network_output_names_.size());
...
nvinfer1::Weights TensorRTBuilder::GetDLTensorAsWeights(const DLTensor* dptr,
  ...
  TVM_ICHECK_EQ(dptr->ctx.device_type, src_device);
  TVM_ICHECK(static_cast<int>(dptr->dtype.code) == kDLFloat ||
  ...
  TVM_ICHECK_EQ(count * 4, weight_bytes);
  weight.count = count;
  weight.values = new float[count];
  TVM_ICHECK_EQ(TVMArrayCopyToBytes(const_cast<DLTensor*>(dptr), const_cast<void*>(weight.values),
                                weight_bytes),
            0)
      << TVMGetLastError();
