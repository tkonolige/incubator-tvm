...
void CHECK_CUSPARSE_ERROR(cusparseStatus_t status) {
  if (status != CUSPARSE_STATUS_SUCCESS) {
    if (status == CUSPARSE_STATUS_NOT_INITIALIZED) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_NOT_INITIALIZED";
    } else if (status == CUSPARSE_STATUS_ALLOC_FAILED) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_ALLOC_FAILED";
    } else if (status == CUSPARSE_STATUS_INVALID_VALUE) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_INVALID_VALUE";
    } else if (status == CUSPARSE_STATUS_ARCH_MISMATCH) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_ARCH_MISMATCH";
    } else if (status == CUSPARSE_STATUS_EXECUTION_FAILED) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_EXECUTION_FAILED";
    } else if (status == CUSPARSE_STATUS_INTERNAL_ERROR) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_INTERNAL_ERROR";
    } else if (status == CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_MATRIX_TYPE_NOT_SUPPORTED";
    } else if (status == CUSPARSE_STATUS_NOT_SUPPORTED) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_NOT_SUPPORTED";
    } else if (status == CUSPARSE_STATUS_INSUFFICIENT_RESOURCES) {
      LOG(FATAL) << "CUSparse error: CUSPARSE_STATUS_INSUFFICIENT_RESOURCES";
    } else {
      LOG(FATAL) << "CUSparse error: unknown error number " << status;
    }
  }
}
