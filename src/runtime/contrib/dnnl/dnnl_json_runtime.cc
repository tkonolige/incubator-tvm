...
class DNNLJSONRuntime : public JSONRuntimeBase {
 ...
 private:
  ...
  void BuildEngine() {
    ...
    for (size_t nid = 0; nid < nodes_.size(); ++nid) {
      ...
      if (node.GetOpType() == "kernel") {
        TVM_ICHECK_EQ(node.GetOpType(), "kernel");
  ...
  dnnl::memory BindDNNLMemory(const JSONGraphNodeEntry& entry, dnnl::memory mem,
    ...
    TVM_ICHECK_EQ(entry_out_mem_.count(eid), 0);
  ...
  void Conv2d(const size_t& nid, const bool has_relu = false, const bool has_bias = false) {
    ...
    // Data memory.
    TVM_ICHECK_EQ(node.GetAttr<std::vector<std::string>>("data_layout")[0], "NCHW");
    auto conv2d_src_memory = BindDNNLMemory(data_entry, {src_dims, dt::f32, tag::nchw});

    // Weight memory.
    TVM_ICHECK_EQ(node.GetAttr<std::vector<std::string>>("kernel_layout")[0], "OIHW");
  ...
  void Relu(const size_t& nid) {
    ...
    TVM_ICHECK(data_md == relu_prim_desc.dst_desc());
  ...
  void Add(const size_t& nid) {
    ...
    TVM_ICHECK_EQ(node.GetInputs().size(), 2U);
    ...
    TVM_ICHECK(data_dims[0] == data_dims[1]);
  ...
  inline dnnl::memory::desc GenDNNLMemDescByShape(const dnnl::memory::dims& shape, dt dtype) {
    ...
    switch (shape.size()) {
      ...
      case 5:
        ...
        break;
      default:
        TVM_LOG(FATAL) << "Unsupported data shape dimension: " << shape.size();
        break;
    }
    return data_md;
  }
