...
TVM_REGISTER_GLOBAL("tvm.contrib.nnpack.convolution_inference")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      std::call_once(flag, []() { TVM_ICHECK_EQ(nnp_initialize(), nnp_status_success); });
      ...
      TVM_ICHECK_EQ(input->ndim, 4);
      TVM_ICHECK_EQ(kernel->ndim, 4);
      if (bias) {
        TVM_ICHECK_EQ(bias->ndim, 1);
      }
      TVM_ICHECK_EQ(output->ndim, 4);
      TVM_ICHECK_EQ(input->shape[1], kernel->shape[1]);
      TVM_ICHECK_EQ(input->shape[0], output->shape[0]);
      size_t input_channels = input->shape[1];
      TVM_ICHECK_EQ(output->shape[1], kernel->shape[0]);
      if (bias) {
        TVM_ICHECK_EQ(output->shape[1], bias->shape[0]);
      }
      ...
      TVM_ICHECK(input->strides == nullptr);
      TVM_ICHECK(kernel->strides == nullptr);
      if (bias) {
        TVM_ICHECK(bias->strides == nullptr);
      }

      TVM_ICHECK(TypeMatch(input->dtype, kDLFloat, 32));
      TVM_ICHECK(TypeMatch(kernel->dtype, kDLFloat, 32));
      if (bias) {
        TVM_ICHECK(TypeMatch(bias->dtype, kDLFloat, 32));
      }
      TVM_ICHECK(TypeMatch(output->dtype, kDLFloat, 32));
      ...
      TVM_ICHECK_EQ(status, nnp_status_success);
      ...
      TVM_ICHECK(workspace_buffer != nullptr);
...
TVM_REGISTER_GLOBAL("tvm.contrib.nnpack.convolution_inference_without_weight_transform")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      TVM_ICHECK(input->strides == nullptr);
      TVM_ICHECK(transformed_kernel->strides == nullptr);
      if (bias) {
        TVM_ICHECK(bias->strides == nullptr);
      }

      TVM_ICHECK(TypeMatch(input->dtype, kDLFloat, 32));
      TVM_ICHECK(TypeMatch(transformed_kernel->dtype, kDLFloat, 32));
      if (bias) {
        TVM_ICHECK(TypeMatch(bias->dtype, kDLFloat, 32));
      }
      TVM_ICHECK(TypeMatch(output->dtype, kDLFloat, 32));
...
TVM_REGISTER_GLOBAL("tvm.contrib.nnpack.convolution_inference_weight_transform")
    .set_body([](TVMArgs args, TVMRetValue* ret) {
      ...
      TVM_ICHECK_EQ(kernel->shape[2], 3);
      TVM_ICHECK_EQ(kernel->shape[3], 3);
      nnp_size kernel_size{static_cast<size_t>(kernel->shape[2]),
                           static_cast<size_t>(kernel->shape[3])};
      TVM_ICHECK(kernel->strides == nullptr);
      TVM_ICHECK(TypeMatch(kernel->dtype, kDLFloat, 32));
      ...
      TVM_ICHECK_LE(transformed_kernel_size, GetDataSize(*transformed_kernel));
