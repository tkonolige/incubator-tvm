...
struct NDArrayContainerTrait {
  ...
  static void SHashReduce(const runtime::NDArray::Container* key, SHashReducer hash_reduce) {
    TVM_ICHECK_EQ(key->dl_tensor.ctx.device_type, kDLCPU) << "can only compare CPU tensor";
    TVM_ICHECK(runtime::IsContiguous(key->dl_tensor)) << "Can only hash contiguous tensor";
  ...
  static bool SEqualReduce(const runtime::NDArray::Container* lhs,
    ...
    TVM_ICHECK_EQ(lhs->dl_tensor.ctx.device_type, kDLCPU) << "can only compare CPU tensor";
    TVM_ICHECK_EQ(rhs->dl_tensor.ctx.device_type, kDLCPU) << "can only compare CPU tensor";
    TVM_ICHECK(runtime::IsContiguous(lhs->dl_tensor)) << "Can only compare contiguous tensor";
    TVM_ICHECK(runtime::IsContiguous(rhs->dl_tensor)) << "Can only compare contiguous tensor";
...
TVM_REGISTER_GLOBAL("node.ArrayGetItem").set_body([](TVMArgs args, TVMRetValue* ret) {
  int64_t i = args[1];
  TVM_ICHECK_EQ(args[0].type_code(), kTVMObjectHandle);
  Object* ptr = static_cast<Object*>(args[0].value().v_handle);
  TVM_ICHECK(ptr->IsInstance<ArrayNode>());
  auto* n = static_cast<const ArrayNode*>(ptr);
  TVM_ICHECK_LT(static_cast<size_t>(i), n->size()) << "out of bound of array";
  *ret = n->at(i);
});
...
TVM_REGISTER_GLOBAL("node.MapSize").set_body([](TVMArgs args, TVMRetValue* ret) {
  TVM_ICHECK_EQ(args[0].type_code(), kTVMObjectHandle);
  Object* ptr = static_cast<Object*>(args[0].value().v_handle);
  TVM_ICHECK(ptr->IsInstance<MapNode>());
...
TVM_REGISTER_GLOBAL("node.MapGetItem").set_body([](TVMArgs args, TVMRetValue* ret) {
  ...
  TVM_ICHECK(it != n->end()) << "cannot find the corresponding key in the Map";
  *ret = (*it).second;
});
