  ...
  inline R operator()(const Call& call) {
    ...
    if (it != match_map_.end()) {
      ...
      if (default_ != nullptr) {
        ...
        TVM_LOG(FATAL) << "unexpected operation " << call->op;
      }
    }
  }
...
inline void GetPaddingWidth(const Array<IndexExpr>& padding, IndexExpr* pad_w) {
  if (padding.size() == 1) {
    ...
    *pad_w = padding[0] + padding[1];
    ...
    TVM_ICHECK_EQ(padding.size(), 4) << " Expected padding size of 1 or 2, found " << padding.size();
  }
}
...
inline void GetPaddingHeightWidth(const Array<IndexExpr>& padding, IndexExpr* pad_h,
  ...
  if (padding.size() == 1) {
    ...
    *pad_w = padding[1] + padding[3];
    ...
    TVM_ICHECK_EQ(padding.size(), 4) << " Padding size should be 1, 2 or 4, but got " << padding.size();
  }
}
...
inline void GetPaddingDepthHeightWidth(const Array<IndexExpr>& padding, IndexExpr* pad_d,
  ...
  if (padding.size() == 1) {
    ...
    *pad_w = padding[2] + padding[5];
    ...
    TVM_ICHECK_EQ(padding.size(), 6) << " Padding size should be 1, 3 or 6, but got " << padding.size();
  }
}
