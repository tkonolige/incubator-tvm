...
bool ROIAlignRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  TVM_ICHECK(roi_align_attrs);
  TVM_ICHECK_EQ(dshape.size(), 4) << "Input data should be 4-D.";
  TVM_ICHECK_EQ(rshape.size(), 2) << "Input rois should be 2-D.";
  // assign output type
  ...
  if (roi_align_attrs->layout == "NCHW") {
    ...
    TVM_ICHECK_EQ(roi_align_attrs->layout, "NHWC") << "Unexpected ROI Align layout";
...
bool ROIPoolRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  if (roi_pool_attrs->layout == "NCHW") {
    ...
    TVM_LOG(FATAL) << "vision.roi_pool does not support " << roi_pool_attrs->layout << " layout";
  }
