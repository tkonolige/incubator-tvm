...
Array<Array<Layout> > PoolInferCorrectLayout(const Attrs& attrs,
  ...
  if (new_in_layouts.defined()) {
    ...
    TVM_ICHECK_EQ(new_in_layouts.size(), 1);
...
bool Pool2DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  TVM_ICHECK_GE(dshape.size(), 2U)
      << "Pool2D only support input >= 2-D: input must have height and width";
  const auto param = attrs.as<AttrType>();
  TVM_ICHECK(param != nullptr);

  Layout layout(param->layout);
  TVM_ICHECK(layout.Contains(LayoutAxis::Get('H')) && layout.Contains(LayoutAxis::Get('W')) &&
...
Array<te::Tensor> Pool2DCompute(const Attrs& attrs, const Array<te::Tensor>& inputs,
  ...
  TVM_ICHECK(tir::BijectiveLayout(layout, kNCHW).defined())
      << "max_pool2d currently only supports layouts that are convertible from NCHW";
  TVM_ICHECK_EQ(layout.IndexOf(LayoutAxis::Get('h')), -1)
      << "max_pool2d does not support input split on height";
  TVM_ICHECK_EQ(layout.IndexOf(LayoutAxis::Get('w')), -1)
      << "max_pool2d does not support input split on width";

  TVM_ICHECK(inputs[0].ndim() == 4U || inputs[0].ndim() == 5U || inputs[0].ndim() == 6U)
...
Array<te::Tensor> GlobalPool2DCompute(const Attrs& attrs, const Array<te::Tensor>& inputs,
  ...
  TVM_ICHECK(inputs[0].ndim() == 4U || inputs[0].ndim() == 5U)
      << "Pool2D only support 4-D input (e.g., NCHW)"
...
bool AdaptivePool2DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  TVM_ICHECK_LE(output_size.size(), 2U) << "output_size can have up to 2 elements.";
...
bool AdaptivePool3DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  Array<IndexExpr> oshape(dshape);
  auto output_size = param->output_size;
  TVM_ICHECK_LE(output_size.size(), 3U) << "output_size can have up to 3 elements.";
...
Array<te::Tensor> AdaptivePool3DCompute(const Attrs& attrs, const Array<te::Tensor>& inputs,
  ...
  TVM_ICHECK(param != nullptr);
  Layout layout(param->layout);
  TVM_ICHECK(tir::BijectiveLayout(layout, kNCDHW).defined())
      << "Adaptive pool3d currently only supports layouts that are convertible from NCDHW";
  TVM_ICHECK_EQ(layout.IndexOf(LayoutAxis::Get('d')), -1)
  ...
  TVM_ICHECK(inputs[0].ndim() == 5U || inputs[0].ndim() == 6U)
...
Array<te::Tensor> Pool2DGradCompute(const Attrs& attrs, const Array<te::Tensor>& inputs,
  ...
  TVM_ICHECK(inputs[1].ndim() == 4U || inputs[1].ndim() == 5U)
...
bool Pool1DRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  const auto dshape = data->shape;
  TVM_ICHECK_GE(dshape.size(), 1U) << "Pool1D only support input >= 1-D: input must have width";
  const auto param = attrs.as<AttrType>();
  TVM_ICHECK(param != nullptr);

  Layout layout(param->layout);
  TVM_ICHECK(layout.Contains(LayoutAxis::Get('W')) && !layout.Contains(LayoutAxis::Get('w')))
