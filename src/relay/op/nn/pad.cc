  ...
  if (new_in_layouts.defined()) {
    ...
    for (auto iter_var : new_in_layouts[0]->axes) {
      ...
      if (axis_pad_width.count(axis_name) != 0 && new_layout_axis.IsPrimal()) {
        ...
        TVM_ICHECK(axis_pad_width.count(dual_axis_name))
...
bool PadRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  TVM_ICHECK(data->shape.size() == param->pad_width.size())
  ...
  for (size_t i = 0; i < param->pad_width.size(); i++) {
    TVM_ICHECK(param->pad_width[i].size() == 2)
    ...
    TVM_ICHECK(width1 != nullptr);
    TVM_ICHECK(width2 != nullptr);

    TVM_ICHECK(*width1 >= 0) << "Param width elements should be positive but first pad width at "
                         << "index " << i << " is " << *width1 << ".";
    TVM_ICHECK(*width2 >= 0) << "Param width elements should be positive but first pad width at "
                         << "index " << i << " is " << *width2 << ".";
...
Array<te::Tensor> PadCompute(const Attrs& attrs, const Array<te::Tensor>& inputs,
  ...
  TVM_ICHECK(pad_width.size() == inputs[0].ndim() && pad_width[0].size() == 2) << "Illegal pad_width";
