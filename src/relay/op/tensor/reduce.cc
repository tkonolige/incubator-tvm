...
inline std::vector<int64_t> GetReduceAxes(const uint32_t indim, const Array<Integer>& inaxis,
  ...
  for (auto i : inaxis) {
    ...
    TVM_ICHECK(axis >= 0) << "Axis out of bounds in reduce operator.";
    TVM_ICHECK(axis < indim) << "Axis out of bounds in reduce operator.";
    in_axes.push_back(axis);
  }

  TVM_ICHECK(in_axes[in_axes.size() - 1] < indim)
...
Array<Integer> GetExcludeAxes(size_t indim, const Array<Integer>& inaxis) {
  TVM_ICHECK(inaxis.defined()) << "Cannot set exclude when axis=None";
  ...
  for (auto i : inaxis) {
    ...
    // Check out of bounds error
    TVM_ICHECK_GE(axis, 0) << "Axis out of bounds in reduce operator.";
    TVM_ICHECK_LT(axis, static_cast<int64_t>(indim)) << "Axis out of bounds in reduce operator.";
...
Array<Array<Layout>> ReduceInferCorrectLayout(const Attrs& attrs,
  ...
  if (new_in_layouts.defined() && r_axes.size()) {
    ...
    TVM_ICHECK_EQ(old_in_layouts.size(), 1);
...
bool VarianceRel(const Array<Type>& types, int num_inputs, const Attrs& attrs,
  ...
  TVM_ICHECK_EQ(in_shape.size(), mean_shape.size());
