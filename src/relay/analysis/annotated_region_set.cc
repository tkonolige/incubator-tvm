 ...
 public:
  ...
  void AddToArgRegion(Expr expr, Array<Expr> args) {
    ...
    for (auto arg : args) {
      ...
      TVM_ICHECK_EQ(region.defined(), arg_region.defined())
  ...
  void VisitExpr_(const CallNode* call) {
    ...
    if (op_node == nullptr || call->attrs.as<CompilerAttrs>() == nullptr) {
      ...
      if (!region.defined()) {
        throw CompileError(ErrorBuilder() << "Cannot find the corresponding region for end annotation:\n"
        ...
        TVM_ICHECK_EQ(region->GetTarget(), target);
      }
      region->nodes_.insert(GetRef<Call>(call));
