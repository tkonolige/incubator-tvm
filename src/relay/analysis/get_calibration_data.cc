 ...
 public:
  ...
  Expr Rewrite_(const CallNode* call, const Expr& post) final {
    ...
    if (call->op->IsInstance<GlobalVarNode>()) {
      ...
      TVM_ICHECK(module_->ContainGlobalVar(var->name_hint)) << "Function " << var << " is not defined";
 ...
 public:
  ...
  Expr Rewrite_(const CallNode* call, const Expr& post) final {
    if (call->op->IsInstance<GlobalVarNode>()) {
      auto var = Downcast<GlobalVar>(call->op);
      TVM_ICHECK(module_->ContainGlobalVar(var->name_hint)) << "Function " << var << " is not defined";
      TVM_ICHECK_EQ(output_map_->count(var), 0)
