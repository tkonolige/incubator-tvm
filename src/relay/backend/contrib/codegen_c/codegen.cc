...
class CodegenC : public MemoizedExprTranslator<std::vector<Output>>, public CodegenCBase {
 public:
  ...
  std::vector<Output> VisitExprDefault_(const Object* op) final {
    TVM_LOG(FATAL) << "C codegen doesn't support: " << op->GetTypeKey();
    return {};
  }
  ...
  std::vector<Output> VisitExpr_(const ConstantNode* cn) final {
    ...
    TVM_ICHECK(dtype == "float" || dtype == "int") << "Only float and int are supported for now.";
    output.dtype = dtype;
