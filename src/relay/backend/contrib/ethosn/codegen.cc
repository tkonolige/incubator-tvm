...
std::map<Expr, std::vector<sl::TensorInfo>> InferTensorsVisitor::Infer(const Expr& expr) {
  tensor_table_.clear();
  TVM_ICHECK(expr->checked_type().defined());
...
void InferTensorsVisitor::VisitExpr_(const TupleNode* tn) {
  ...
  TVM_ICHECK(tensor_table_.find(tuple) != tensor_table_.end());
...
void InferTensorsVisitor::VisitExpr_(const TupleGetItemNode* tgn) {
  ...
  TVM_ICHECK(tensor_table_.find(tg) != tensor_table_.end());
  auto tuple = tg->tuple;
...
runtime::Module EthosnCompiler::CreateRuntimeModule(const ObjectRef& ref) {
  ...
  if (ref->IsInstance<FunctionNode>()) {
    ...
    TVM_ICHECK(name_node.defined()) << "Failed to retrieved external symbol.";
    ...
    TVM_LOG(FATAL) << "The input ref is expected to be a Relay function";
  }
...
runtime::ethosn::OrderedCompiledNetwork EthosnCompiler::CompileEthosnFunc(const IRModule& mod,
  ...
  TVM_ICHECK_GE(compiled_networks.size(), 1) << "Ethos-N compiler failed to compile network";
