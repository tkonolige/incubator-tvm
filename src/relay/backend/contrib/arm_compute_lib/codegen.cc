...
class ACLJSONSerializer : public backend::contrib::JSONSerializer {
 ...
 public:
  ...
  std::vector<JSONGraphNodeEntry> VisitExpr_(const CallNode* cn) override {
    ...
    if (!cn->op.as<FunctionNode>()) {
      TVM_LOG(FATAL) << "Arm Compute Library JSON runtime does not support calls to "
                 << cn->op->GetTypeKey();
    }
    auto fn = cn->op.as<FunctionNode>();
    auto comp = fn->GetAttr<String>(attr::kComposite);
    TVM_ICHECK(comp.defined()) << "Arm Compute Library JSON runtime only supports composite functions.";
    ...
    if (name == "arm_compute_lib.conv2d" || name == "arm_compute_lib.qnn_conv2d") {
      ...
      TVM_LOG(FATAL) << "Unrecognized Arm Compute Library pattern: " << name;
    }
 ...
 private:
  ...
  static CompositeConvNode UnpackCompositeConvolution(const CallNode* cn) {
    ...
    if (nodes.requantize) {
      TVM_ICHECK(backend::IsOp(current_call, "qnn.conv2d"));
    } else {
      TVM_ICHECK(backend::IsOp(current_call, "nn.conv2d"));
    }
  ...
  std::shared_ptr<JSONGraphNode> CreateCompositeConvJSONNode(const CallNode* cn) {
    ...
    TVM_ICHECK(conv_attr);
    TVM_ICHECK(conv_attr->kernel_layout == "OHWI")
  ...
  static CompositeDenseNode UnpackCompositeDense(const CallNode* cn) {
    ...
    if (nodes.requantize) {
      TVM_ICHECK(backend::IsOp(current_call, "qnn.dense"));
    } else {
      TVM_ICHECK(backend::IsOp(current_call, "nn.dense"));
    }
  ...
  std::shared_ptr<JSONGraphNode> CreateCompositeL2Pool2DJSONNode(const CallNode* cn) {
    ...
    TVM_ICHECK(exponent);
    TVM_ICHECK_EQ(*static_cast<float*>(exponent->data->data), 2) << "Exponent must be 2 for L2 pooling";
