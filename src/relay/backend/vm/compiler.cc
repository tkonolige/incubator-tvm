...
Pass ManifestAlloc(Target target_host, vm::TargetsMap targets) {
  ...
  TVM_ICHECK(f != nullptr) << "unable to load allocation manifestation pass";
  return (*f)(target_host, targets);
}

Pass MemoryPlan() {
  auto f = tvm::runtime::Registry::Get("relay.transform.MemoryPlan");
  TVM_ICHECK(f != nullptr) << "unable to load the memory planning pass";
  return (*f)();
}

Pass LiftConstants() {
  auto f = tvm::runtime::Registry::Get("relay.transform.LiftConstants");
  TVM_ICHECK(f != nullptr) << "unable to load the constant lifting pass";
  return (*f)();
}
...
TreeObjectPtr BuildDecisionTreeFromPattern(MatchValuePtr data, Pattern pattern,
  ...
  if (pattern.as<PatternWildcardNode>()) {
    ...
    TVM_ICHECK(pt) << "unhandled case: " << AsText(pattern, false);
    size_t field_index = 0;
...
std::vector<int64_t> ToAllocTensorShape(NDArray shape) {
  ...
  TVM_ICHECK_EQ(shape->ndim, 1u);
  TVM_ICHECK_EQ(shape->dtype.code, 0U) << "The dtype of constant shape must be int32 or int64, but got "
                                   << DLDataType2String(shape->dtype);
  TVM_ICHECK(shape->dtype.bits == 64 || shape->dtype.bits == 32)
...
class VMFunctionCompiler : ExprFunctor<void(const Expr& expr)> {
  ...
  VMFunction Compile(const GlobalVar& var, const Function& func) {
    ...
    for (const auto& it : func->params) {
      if (!expr_device_map_.empty()) {
        TVM_ICHECK_GT(expr_device_map_.count(it), 0U);
        params_device_type.push_back(expr_device_map_[it].device_type);
      } else {
        TVM_ICHECK_EQ(targets_.size(), 1U);
 ...
 protected:
  ...
  inline void Emit(const Instruction& instr) {
    ...
    TVM_ICHECK((int)instr.op < 100) << "Invalid opcode " << (int)instr.op;
    switch (instr.op) {
  ...
  void VisitExpr_(const ConstantNode* const_node) {
    ...
    if (expr_device_map_.empty()) {
      ...
      TVM_ICHECK_GT(expr_device_map_.count(con), 0U);
  ...
  void VisitExpr_(const VarNode* var_node) {
    ...
    TVM_ICHECK(reg_it != this->var_register_map_.end());
  ...
  void VisitExpr_(const GlobalVarNode* gvar) {
    ...
    TVM_ICHECK(it != context_->global_map.end());
  ...
  void EmitShapeFunc(Function func, Array<Expr> inputs, Array<Expr> outputs) {
    ...
    TVM_ICHECK_EQ(cfunc->funcs->functions.size(), 1);
    ...
    for (auto output : outputs) {
      ...
      TVM_ICHECK(reg != var_register_map_.end())
  ...
  void EmitInvokeTVMOp(const Function& func, const Expr& inputs, const Expr& outputs) {
    ...
    if (func->GetAttr<String>(attr::kCompiler).defined()) {
      ...
      if (expr_device_map_.empty()) {
        ...
        TVM_ICHECK_GT(expr_device_map_.count(func), 0U)
  ...
  void VisitExpr_(const CallNode* call_node) {
      ...
      matcher
          ...
          .Match("memory.alloc_tensor",
                 [this](const Array<Expr>& args, const Attrs& attrs, const Array<Type>& type_arg) {
                   ...
                   TVM_ICHECK(alloc_attrs != nullptr) << "must be the alloc tensor attrs";
          ...
          .Match("memory.alloc_storage",
                 [this, call_node](const Array<Expr>& args, const Attrs& attrs,
                   ...
                   TVM_ICHECK_EQ(args.size(), 2);
                   // Compute the size of the allocation.
                   this->VisitExpr(args[0]);
                   auto size_register = last_register_;

                   TVM_ICHECK(args[1].as<ConstantNode>());
                   NDArray alignment_arr = args[1].as<ConstantNode>()->data;
                   TVM_ICHECK_EQ(alignment_arr->dtype.code, 0U)
                       << "The dtype of constant shape must be int32 or int64, but got "
                       << DLDataType2String(alignment_arr->dtype);
                   TVM_ICHECK_EQ(alignment_arr->dtype.bits, 64U);
                   ...
                   TVM_ICHECK(alloc_attrs != nullptr) << "must be the AllocStorage attrs";
                   auto dtype = alloc_attrs->dtype;
                   ...
                   if (expr_device_map_.empty()) {
                     ...
                     TVM_ICHECK_GT(expr_device_map_.count(GetRef<Call>(call_node)), 0U)
          ...
          .Match("vm.shape_of",
                 [this](const Array<Expr>& args, const Attrs& attrs, const Array<Type>& type_arg) {
                   ...
                   TVM_ICHECK(shape_of_attrs) << "Must be the shape_of attrs";
                   TVM_ICHECK_EQ(shape_of_attrs->dtype.bits(), 64)
          ...
          .Match("device_copy",
                 [this](const Array<Expr>& args, const Attrs& attrs, const Array<Type>& type_arg) {
                   ...
                   TVM_ICHECK(device_copy_attrs != nullptr) << "Must be the device copy attrs";
          ...
          .Match("memory.kill",
                 [](const Array<Expr>& args, const Attrs& attrs, const Array<Type>& type_arg) {
                   TVM_LOG(FATAL) << "memory.kill is not yet supported";
                 });
    ...
    if (auto global_node = op.as<GlobalVarNode>()) {
      ...
      TVM_LOG(FATAL) << "internal error: unreachable code,"
  ...
  void VisitExpr_(const FunctionNode* func_node) {
    if (!func_node->HasNonzeroAttr(attr::kPrimitive)) {
      TVM_LOG(FATAL) << "local functions should have been removed by lambda lifting:" << std::endl
...
void VMCompiler::Lower(IRModule mod, const TargetsMap& targets, const tvm::Target& target_host) {
  if (params_.size()) {
    ...
    TVM_ICHECK(base_func->IsInstance<FunctionNode>())
  ...
  for (auto named_func : context_.module->functions) {
    ...
    if (auto* n = named_func.second.as<FunctionNode>()) {
      ...
      TVM_ICHECK(func_index < exec_->functions.size());
...
void VMCompiler::Codegen() {
  if (!context_.module.defined()) {
    TVM_LOG(WARNING) << "Did you forget to call VMCompiler::Lower?";
    return;
  }
  ...
  for (auto& cfunc : cached_funcs) {
    ...
    if (target_str == "ext_dev") {
      ...
      TVM_ICHECK(mod->ContainGlobalVar(cfunc->func_name));
