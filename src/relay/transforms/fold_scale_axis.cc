...
class ScaledExprNode : public TempExprNode {
 public:
  ...
  Expr Realize() const final {
    TVM_ICHECK(!axes.defined()) << "outstanding scale";
    return value;
  }
  ...
  void VisitExpr_(const CallNode* call) {
    ...
    auto flazy = [this, call]() {
      ...
      if (f != nullptr) {
        ...
        TVM_ICHECK_EQ(in_messages.size(), call->args.size());
...
Expr AddSubForwardRewrite(const Call& ref_call, const Array<Expr>& new_args,
  ...
  if (slhs != nullptr) {
    TVM_ICHECK(srhs == nullptr);
    TVM_ICHECK(MatchBroadcastToLeftAxes(tlhs, trhs, slhs->axes));
    ...
    TVM_ICHECK(srhs != nullptr);
    TVM_ICHECK(MatchBroadcastToLeftAxes(trhs, tlhs, srhs->axes));
...
class BackwardPrep : private ExprVisitor {
 ...
 private:
  ...
  void VisitExpr_(const CallNode* call) {
    ...
    TVM_ICHECK(rit != ref_counter_.end());
 ...
 public:
  ...
  Expr Transform(const Expr& expr, Message message, Expr scale) {
    ...
    if (const CallNode* call_node = expr.as<CallNode>()) {
      ...
      TVM_ICHECK(!message.defined()) << "outstanding scale";
...
Expr AddSubBackwardTransform(const Call& call, const Message& message, const Expr& scale,
  ...
  if (lhs_message.defined() && rhs_message.defined()) {
    TVM_ICHECK(equal(lhs_message->axes, rhs_message->axes));
    TVM_ICHECK(equal(message->axes, lhs_message->axes));
    ...
    TVM_ICHECK(equal(message->axes, rhs_message->axes));
    ...
    TVM_LOG(FATAL) << "outstanding scale";
    return Expr();
  }
}
...
Expr MultiplyBackwardTransform(const Call& call, const Message& message, const Expr& scale,
  ...
  if (lhs_message.defined()) {
    TVM_ICHECK(lhs_message->axes.defined() && lhs_message->axes.size());
    ...
    TVM_ICHECK(rhs_message->axes.defined() && rhs_message->axes.size());
