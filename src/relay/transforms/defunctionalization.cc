...
class DefuncMutator : public ExprMutator {
 public:
  ...
  Expr VisitExpr_(const CallNode* call) {
    if (auto op = call->op.as<GlobalVarNode>()) {
      TVM_ICHECK_EQ(call->type_args.size(), op->checked_type().as<FuncTypeNode>()->type_params.size())
          << "all type args must be explicit";

      auto op_type = InstFuncType(op->checked_type().as<FuncTypeNode>(), call->type_args);
      TVM_ICHECK_EQ(FreeTypeVars(op_type, mod).size(), 0) << "free type vars in instantiated";
      TVM_ICHECK(!HasFuncType(op_type->ret_type)) << "returning functions not supported";
      ...
      TVM_ICHECK(var_original_type) << "var original type not saved in var_save_type map";
 ...
 private:
  ...
  void AddApplyCase(GlobalVar apply_gv, FuncType ft, Constructor c, const Expr& expr,
    ...
    TVM_ICHECK(c->inputs.size() == patterns.size())
    ...
    if (!mod->ContainGlobalVar(apply_gv->name_hint)) {
      ...
      TVM_ICHECK(body) << "internal invariant broken; apply function body should be a match node";
  ...
  Expr EncodeArg(const Expr& arg, const Type& type) {
    ...
    TVM_ICHECK(type.as<FuncTypeNode>()) << "assume no nested functions";
    TVM_ICHECK(arg.as<VarNode>() || arg.as<GlobalVarNode>() || arg.as<FunctionNode>())
  ...
  FuncType GetUnencodedType(const Type& t) {
    ...
    TVM_ICHECK(tc) << "expected type call when getting original type from encoded type";
    auto gv = tc->func.as<GlobalTypeVarNode>();
    TVM_ICHECK(gv) << "expected global type var in encoded type";
    auto type = original_func_type_map[GetRef<GlobalTypeVar>(gv)];
    TVM_ICHECK(type.defined()) << "reverse mapping from encoded type to original type not found";
  ...
  FuncType InstFuncType(const FuncTypeNode* fty, const Array<Type> type_args) {
    TVM_ICHECK(fty) << "InstFuncType functype is null";
    TVM_ICHECK_EQ(fty->type_params.size(), type_args.size())
  ...
  Function Specialize(const Function& f, const Array<Type> type_args) {
    TVM_ICHECK_EQ(f->type_params.size(), type_args.size())
  ...
  Function FirstifyVars(const Function& f) {
    TVM_ICHECK(f->type_params.size() == 0) << "firstify function has type params";
    ...
    for (auto var : f->params) {
      if (auto var_type = var->type_annotation.as<FuncTypeNode>()) {
        ...
        TVM_ICHECK(!HasFuncType(var->type_annotation))
...
Expr Defunctionalization(const Function& f, const IRModule& mod) {
  ...
  TVM_ICHECK(f->type_params.size() == 0) << "no polymorphism supported for defunctionalization";
  for (const auto& p : f->params) {
    TVM_ICHECK(!HasFuncType(p->checked_type())) << "program cannot have func type parameters";
  }
  TVM_ICHECK(!HasFuncType(f->ret_type)) << "return type cannot contain function";
