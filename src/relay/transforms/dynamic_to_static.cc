    ...
    op_map_ = {
        {Op::Get("dyn.reshape"),
         [](const CallNode* call_node) {
           if (const ConstantNode* shape = call_node->args[1].as<ConstantNode>()) {
             TVM_ICHECK_EQ(shape->data->ndim, 1);
        ...
        {Op::Get("dyn.tile"),
         [](const CallNode* call_node) {
           if (const ConstantNode* reps = call_node->args[1].as<ConstantNode>()) {
             TVM_ICHECK_EQ(reps->data->ndim, 1);
        ...
        {Op::Get("dyn.nn.upsampling"),
         [](const CallNode* call_node) {
           ...
           if (scale_h && scale_w) {
             TVM_ICHECK_EQ(scale_h->data->ndim, 0);
             TVM_ICHECK_EQ(scale_w->data->ndim, 0);
        ...
        {Op::Get("dyn.nn.upsampling3d"),
         [](const CallNode* call_node) {
           ...
           if (scale_d && scale_h && scale_w) {
             TVM_ICHECK_EQ(scale_d->data->ndim, 0);
             TVM_ICHECK_EQ(scale_h->data->ndim, 0);
             TVM_ICHECK_EQ(scale_w->data->ndim, 0);
        ...
        {Op::Get("dyn.nn.pad"),
         [](const CallNode* call_node) {
           ...
           if (pad_width && pad_fill) {
             TVM_ICHECK_EQ(pad_fill->data->ndim, 0);   // pad_val is 1d
             TVM_ICHECK_EQ(pad_width->data->ndim, 2);  // pad_width is 2d
        ...
        {Op::Get("dyn.strided_slice"),
         [](const CallNode* call_node) {
           ...
           if (begin && end && stride) {
             TVM_ICHECK_EQ(begin->data->ndim, 1);
             TVM_ICHECK_EQ(end->data->ndim, 1);
             TVM_ICHECK_EQ(stride->data->ndim, 1);
        ...
        {Op::Get("dyn.sparse_to_dense"),
         [](const CallNode* call_node) {
           ...
           if (output_shape) {
             TVM_ICHECK_EQ(output_shape->data->ndim, 1);
