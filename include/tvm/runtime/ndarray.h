...
inline void NDArray::CopyFrom(const NDArray& other) {
  TVM_ICHECK(data_ != nullptr);
  TVM_ICHECK(other.data_ != nullptr);
...
inline bool NDArray::Load(dmlc::Stream* strm) {
  ...
  TVM_ICHECK(strm->Read(&header)) << "Invalid DLTensor file format";
  TVM_ICHECK(strm->Read(&reserved)) << "Invalid DLTensor file format";
  TVM_ICHECK(header == kTVMNDArrayMagic) << "Invalid DLTensor file format";
  DLContext ctx;
  int ndim;
  DLDataType dtype;
  TVM_ICHECK(strm->Read(&ctx)) << "Invalid DLTensor file format";
  TVM_ICHECK(strm->Read(&ndim)) << "Invalid DLTensor file format";
  TVM_ICHECK(strm->Read(&dtype)) << "Invalid DLTensor file format";
  TVM_ICHECK_EQ(ctx.device_type, kDLCPU) << "Invalid DLTensor context: can only save as CPU tensor";
  std::vector<int64_t> shape(ndim);
  if (ndim != 0) {
    TVM_ICHECK(strm->ReadArray(&shape[0], ndim)) << "Invalid DLTensor file format";
  }
  ...
  TVM_ICHECK(strm->Read(&data_byte_size)) << "Invalid DLTensor file format";
  TVM_ICHECK(data_byte_size == num_elems * elem_bytes) << "Invalid DLTensor file format";
  auto read_ret = strm->Read(ret->data, data_byte_size);
  // Only check non-empty data
  if (ndim > 0 && shape[0] != 0) {
    TVM_ICHECK(read_ret) << "Invalid DLTensor file format";
  }
  if (!DMLC_IO_NO_ENDIAN_SWAP) {
