...
inline Tensor lrn(const Tensor& data, int size, int axis = 1, float alpha = 0.0001,
  ...
  TVM_ICHECK_EQ(data->shape.size(), 4) << "LRN requires 4-D input";
  TVM_ICHECK_EQ(size % 2, 1) << "size should be odd number";
  TVM_ICHECK(axis == 1 || axis == 3) << "axis should be 1 or 3 for NCHW and NHWC";
